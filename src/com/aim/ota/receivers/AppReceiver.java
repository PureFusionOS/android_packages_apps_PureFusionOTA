...
import com.aim.ota.AIMOTA;
import com.aim.ota.R;
import com.aim.ota.RomUpdate;
import com.aim.ota.activities.AddonActivity;
import com.aim.ota.activities.AvailableActivity;
import com.aim.ota.tasks.LoadUpdateManifest;
import com.aim.ota.utils.Constants;
import com.aim.ota.utils.Preferences;
import com.aim.ota.utils.Utils;
...
public class AppReceiver extends BroadcastReceiver implements Constants{
	...
	public void onReceive(Context context, Intent intent) {
		...
		if (action.equals(DownloadManager.ACTION_DOWNLOAD_COMPLETE)) {
			...
			Set<Integer> set = AIMOTA.getAddonDownloadKeySet();
			Iterator<Integer> iterator = set.iterator();

			while (iterator.hasNext() && isAddonDownload != true) {
				int nextValue = iterator.next();
				if (id == AIMOTA.getAddonDownload(nextValue)) {
			...
			if (isAddonDownload) {
				...
				if (DownloadManager.STATUS_SUCCESSFUL != cursor.getInt(statusIndex)) {
					...
					AIMOTA.removeAddonDownload(keyForAddonDownload);
