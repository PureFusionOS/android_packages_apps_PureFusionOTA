...
def valid(names):
    ...
    def valid_one(name):
        return __all_features.has_key(name)
...
def implied_subfeature(feature, subvalue, value_string):
    ...
    if not result:
        raise InvalidValue(
            "'%s' is not a known subfeature value of '%s%s'" % (subvalue, feature, value_string))
    ...
    if feature.implicit():
        for v in values:
            if __implicit_features.has_key(v):
                raise BaseException("'%s' is already associated with the feature '%s'" % (
...
def compose(composite_property_s, component_properties_s):
    ...
    if __composite_properties.has_key(property):
        ...
        composite_property, str(__composite_properties[composite_property])))

    if composite_property in component_properties:
        raise BaseException(
            'composite property "%s" cannot have itself as a component' % composite_property)
...
def minimize(properties):
    ...
    while properties:
        ...
        if subproperties:
            ...
            subproperties.sort()
            joined = b2.build.property.Property(p.feature(), p.value() + '-' + '-'.join(
                [sp.value() for sp in subproperties]))
...
def __validate_feature_attributes(name, attributes):
    for attribute in attributes:
        if not attribute in __all_attributes:
            raise InvalidAttribute("unknown attributes: '%s' in feature declaration: '%s'" % (
            str(b2.util.set.difference(attributes, __all_attributes)), name))
    ...
    if name in __all_features:
        ...
        raise InvalidAttribute(
            "free features cannot also be implicit (in declaration of feature '%s')" % name)
    elif 'free' in attributes and 'propagated' in attributes:
        raise InvalidAttribute(
            "free features cannot also be propagated (in declaration of feature '%s')" % name)
