...
class TargetRegistry:
    ...
    def create_typed_target(self, type, project, name, sources, requirements, default_build,
                            usage_requirements):
        ...
        return self.main_target_alternative(TypedTarget(name, project, type,
                                                        ...
                                                        self.main_target_requirements(requirements,
                                                                                      project),
                                                        self.main_target_default_build(
                                                            default_build, project),
                                                        self.main_target_usage_requirements(
                                                            usage_requirements, project)))
...
class ProjectTarget(AbstractTarget):
    ...
    def add_alternative(self, target_instance):
        ...
        if self.built_main_targets_:
            raise IllegalOperation(
                "add-alternative called when main targets are already created for project '%s'" % self.full_name())
    ...
    def find(self, id, no_error=False):
        ...
        raise BaseException(
            "Unable to find file or target named '%s'\nreferred from project at '%s'" % (
            id, self.get('location')))
    ...
    def add_alternative(self, target):
        ...
        if self.alternatives_ and self.default_build_ != d:
            get_manager().errors()("default build must be identical in all alternatives\n"
                                   ...
                                   self.full_name(), d.raw(), self.default_build_.raw()))
...
class BasicTarget(AbstractTarget):
    ...
    def __init__(self, name, project, sources, requirements=None, default_build=None,
        ...
        for s in sources:
            if get_grist(s):
                raise InvalidSource(
                    "property '%s' found in the 'sources' parameter for '%s'" % (s, name))
    ...
    def compute_usage_requirements(self, subvariant):
        ...
        (r1, r2) = self.generate_dependency_properties(xusage_requirements.dependency(),
                                                       rproperties)
        extra = r1 + r2
        ...
        removed_pch = filter(
            lambda prop: prop.feature().name() not in ['<pch-header>', '<pch-file>'],
            subvariant.sources_usage_requirements().all())
...
def apply_default_build(property_set, default_build):
    ...
    if defaults_to_apply:
        ...
        result = build_request.expand_no_defaults(
            b2.build.property_set.create(feature.expand([p])) for p in
            (compressed + defaults_to_apply))
...
def create_metatarget(klass, name, sources, requirements=[], default_build=[],
                      usage_requirements=[]):
    from b2.manager import get_manager
    t = get_manager().targets()
...
def metatarget_function_for_class(class_):
    ...
    def create_metatarget(name, sources, requirements=[], default_build=None,
                          usage_requirements=[]):
        from b2.manager import get_manager
        t = get_manager().targets()
