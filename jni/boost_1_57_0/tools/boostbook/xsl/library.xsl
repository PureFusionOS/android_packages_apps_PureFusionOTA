...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
    <xsl:key name="library-categories" match="library" use="libraryinfo/librarycategory/@name" />

    <xsl:template match="librarylist">
        <itemizedlist spacing="compact">
            <xsl:apply-templates mode="build-library-list" select="//library">
    ...
    <xsl:template match="library" mode="build-library-list">
        <listitem>
            <simpara>
                ...
                <xsl:text>-</xsl:text>
                <xsl:apply-templates mode="build-library-list"
                    select="libraryinfo/librarypurpose" />
            </simpara>
        </listitem>
    </xsl:template>
    ...
    <xsl:template match="librarypurpose" mode="build-library-list">
        ...
        <xsl:apply-templates mode="display-author-list" select="../author" />
    </xsl:template>
    ...
    <xsl:template match="librarycategorydef">
        <section>
            ...
            <itemizedlist spacing="compact">
                <xsl:apply-templates mode="build-library-list"
                    select="key('library-categories', $name)">
                    <xsl:sort select="@name" />
                </xsl:apply-templates>
            </itemizedlist>
        </section>
