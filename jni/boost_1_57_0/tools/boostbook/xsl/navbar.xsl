    ...
    <xsl:template name="header.navigation">
        ...
        <xsl:if test="boolean(normalize-space($boost.image.src)) or $nav.layout != 'none'">
            <table cellpadding="2" width="100%">
                <tr>
                    ...
                    <td valign="top">
                        <xsl:if test="$nav.border = 'Boost'">
                            <xsl:attribute name="style">background-color: white; width: 50%;
                            </xsl:attribute>
                        </xsl:if>
                        <xsl:if test="boolean(normalize-space($boost.image.src))">
                            <img alt="{$boost.image.alt}" height="{$boost.image.h}"
                                width="{$boost.image.w}">
                    ...
                    <xsl:choose>
                        ...
                        <xsl:when test="$nav.layout = 'vertical'">
        ...
        <xsl:choose>
            <xsl:when test="$nav.flow = 'DocBook'">
                <table class="navheader" width="100%">
        ...
        <xsl:variable name="people_link">
            <xsl:call-template name="href.target.relative">
                <xsl:with-param name="target"
                    select="concat( $boost.website, '/users/people.html' )" />
            </xsl:call-template>
        </xsl:variable>
        <xsl:variable name="faq_link">
            <xsl:call-template name="href.target.relative">
                <xsl:with-param name="target"
                    select="concat( $boost.website, '/users/faq.html' )" />
            </xsl:call-template>
        </xsl:variable>
        ...
        <xsl:choose>
            <xsl:when test="$nav.border = 'Boost'">
                <td align="center" class="boost-headtd">
                    <a class="boost-headelem" href="{$home_link}">Home</a>
                </td>
                <xsl:if test="boolean($libraries_link)">
                    <td align="center" class="boost-headtd">
                        <a class="boost-headelem" href="{$libraries_link}">Libraries</a>
                    </td>
                </xsl:if>
                <td align="center" class="boost-headtd">
                    <a class="boost-headelem" href="{$people_link}">People</a>
                </td>
                <td align="center" class="boost-headtd">
                    <a class="boost-headelem" href="{$faq_link}">FAQ</a>
                </td>
                <td align="center" class="boost-headtd">
                    <a class="boost-headelem" href="{$more_link}">More</a>
                </td>
            </xsl:when>
            <xsl:otherwise>
                <td align="center">
                ...
                <td align="center">
                    <a href="{$libraries_link}">Libraries</a>
                </td>
                <td align="center">
                    <a href="{$people_link}">People</a>
                </td>
                <td align="center">
        ...
        <tr>
            <td align="left" width="40%">
                <xsl:if test="$navig.showtitles != 0"> <!-- prev:name -->
                    <xsl:apply-templates mode="object.title.markup" select="$prev" />
                </xsl:if>
            </td>
            <td align="center" width="20%">
                ...
                <xsl:if test="$chunk.tocs.and.lots != 0 and $nav.context != 'toc'">
                    <a accesskey="t">
                        <xsl:attribute name="href">
                            <xsl:apply-templates mode="recursive-chunk-filename" select="/*[1]" />
            ...
            <td align="right" width="40%">
                <xsl:if test="$navig.showtitles != 0"> <!-- next:name -->
                    <xsl:apply-templates mode="object.title.markup" select="$next" />
                </xsl:if>
            </td>
        </tr>
    </xsl:template>
