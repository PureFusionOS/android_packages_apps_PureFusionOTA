...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
    ...
    <xsl:template match="class-specialization|struct-specialization|union-specialization"
        mode="generate.id">
        <xsl:call-template name="fully-qualified-id">
            <xsl:with-param name="node" select="." />
        </xsl:call-template>
    ...
    <xsl:template name="type.display.name">
        <xsl:choose>
            ...
            <xsl:otherwise>
                <xsl:message>
                    Unknown type element "<xsl:value-of select="local-name(.)" />" in
                    type.display.name
                </xsl:message>
            </xsl:otherwise>
        </xsl:choose>
    ...
    <xsl:template match="class|class-specialization|
        ...
        <xsl:variable name="full-decl-string">
            <xsl:apply-templates mode="synopsis" select="template">
        ...
        <xsl:apply-templates mode="header-synopsis" select="free-function-group">
            <xsl:with-param name="indentation" select="$indentation" />
            ...
            <xsl:otherwise>
                ...
                <xsl:variable name="type-length">
                    <xsl:choose>
                        <xsl:when test="@type">
                            <xsl:message>
                                <xsl:text>Warning: `type' attribute of `typedef' element is
                                    deprecated. Use 'type' element instead.
                                </xsl:text>
                            </xsl:message>
                ...
                <xsl:choose>
                    ...
                    <xsl:otherwise>
                        <xsl:apply-templates mode="highlight" select="type/*|type/text()" />
                        <!--
              ...
              1, $max-name-length)" />
                ...
                <xsl:if test="$compact and purpose">
                    ...
                    <xsl:call-template name="highlight-comment">
                        <xsl:with-param name="text">
                            <xsl:text>//</xsl:text>
                            <xsl:apply-templates mode="comment" select="purpose" />
                        </xsl:with-param>
                    </xsl:call-template>
                </xsl:if>
            </xsl:otherwise>
    ...
    <xsl:template match="static-constant" mode="synopsis">
        ...
        <xsl:apply-templates mode="highlight" select="type" />
    ...
    <xsl:template name="print.base.classes.multi">
        ...
        <xsl:for-each select="inherit">
            ...
            <xsl:choose>
                <xsl:when test="type">
                    <xsl:apply-templates mode="annotation" select="type/*|type/text()">
                        <xsl:with-param name="highlight" select="true()" />
                    </xsl:apply-templates>
                </xsl:when>
                <xsl:otherwise>
    ...
    <xsl:template name="class-members-synopsis">
        ...
        <xsl:if test="typedef">
            ...
            <xsl:apply-templates mode="synopsis" select="typedef">
                <xsl:with-param name="indentation" select="$indentation + 2" />
                <xsl:with-param name="max-type-length" select="$max-type-length" />
                <xsl:with-param name="max-name-length" select="$max-name-length" />
        ...
        <xsl:if test="static-constant">
            ...
            <xsl:apply-templates mode="synopsis" select="static-constant">
                <xsl:with-param name="indentation" select="$indentation + 2" />
            </xsl:apply-templates>
        </xsl:if>
        ...
        <xsl:if
            test="class|class-specialization|struct|struct-specialization|union|union-specialization">
            <xsl:text>&#10;</xsl:text>
            ...
            <xsl:apply-templates mode="reference" select="class|class-specialization|
                                 struct|struct-specialization|
        ...
        <!-- Enums -->
        <xsl:apply-templates mode="synopsis" select="enum">
            <xsl:with-param name="indentation" select="$indentation + 2" />
        </xsl:apply-templates>
        ...
        <!-- Member functions -->
        <xsl:apply-templates mode="synopsis" select="method-group|method|overloaded-method">
            <xsl:with-param name="indentation" select="$indentation + 2" />
        </xsl:apply-templates>
        ...
        <xsl:if test="data-member">
            ...
            <xsl:apply-templates mode="synopsis" select="data-member">
                <xsl:with-param name="indentation" select="$indentation + 2" />
            </xsl:apply-templates>
        </xsl:if>
    </xsl:template>
    ...
    <xsl:template name="class-type-synopsis">
        ...
        <xsl:if test="template">
            ...
            <xsl:apply-templates mode="reference" select="template">
                <xsl:with-param name="indentation" select="$indentation" />
            </xsl:apply-templates>
        </xsl:if>
        <xsl:text>&#10;</xsl:text>
        ...
        <xsl:choose>
            <xsl:when test="inherit">
                ...
                <xsl:call-template name="print.base.classes">
                    <xsl:with-param name="indentation" select="string-length($class-key) + string-length(@name)
                    + $indentation + 1" />
        ...
        <xsl:apply-templates mode="synopsis" select="access">
            <xsl:with-param name="indentation" select="$indentation" />
            <xsl:with-param name="allow-synopsis-anchors" select="$allow-synopsis-anchors" />
        </xsl:apply-templates>
    ...
    <xsl:template name="member-typedefs-reference">
        ...
        <xsl:if test="$have-typedef-references">
            <xsl:call-template name="member-documentation">
                ...
                <xsl:with-param name="text">
                    <orderedlist>
                        <xsl:apply-templates mode="reference" select="typedef" />
                    </orderedlist>
                </xsl:with-param>
            </xsl:call-template>
        </xsl:if>
    ...
    <xsl:template name="data-member-reference">
        <xsl:if test="data-member/description">
            <xsl:call-template name="member-documentation">
                ...
                <xsl:with-param name="text">
                    <orderedlist>
                        <xsl:apply-templates mode="reference" select="data-member" />
                    </orderedlist>
                </xsl:with-param>
            </xsl:call-template>
        </xsl:if>
    ...
    <xsl:template name="class-members-reference">
        ...
        <xsl:apply-templates mode="reference" select="method-group|method|overloaded-method" />

        <xsl:call-template name="data-member-reference" />

        <!-- Emit reference docs for nested classes -->
        <xsl:apply-templates mode="namespace-reference" select="class|class-specialization|
              struct|struct-specialization|
              union|union-specialization" />

        <!-- Emit reference docs for nested enums -->
        <xsl:apply-templates mode="namespace-reference" select="enum" />
    </xsl:template>
    ...
    <xsl:template match="class|class-specialization|
        ...
        <xsl:call-template name="reference-documentation">
            ...
            <xsl:with-param name="synopsis">
                ...
                <xsl:apply-templates mode="synopsis" select="free-function-group">
                    <xsl:with-param name="indentation" select="$indentation" />
                    <xsl:with-param name="class" select="@name" />
                </xsl:apply-templates>
            </xsl:with-param>
            <xsl:with-param name="text">
                ...
                <xsl:if test="para">
                    <xsl:message>
                        <xsl:text>Warning: Use of 'para' elements in 'class' element is deprecated.&#10;Wrap
                            them in a 'description' element.
                        </xsl:text>
                    </xsl:message>
                    <xsl:call-template name="print.warning.context" />
                    <xsl:apply-templates mode="annotation" select="para" />
                </xsl:if>
                <xsl:apply-templates select="description" />
                <xsl:call-template name="class-templates-reference" />
                <xsl:call-template name="class-members-reference" />
                <xsl:apply-templates mode="namespace-reference" select="access" />

                <xsl:apply-templates mode="reference" select="free-function-group">
                    <xsl:with-param name="class" select="@name" />
                </xsl:apply-templates>
                ...
                <xsl:if test="local-name(.)='class' and
                    ...
                    <refsect2>
                        ...
                        <itemizedlist>
                            <xsl:apply-templates mode="specialization-list"
    ...
    <xsl:template match="data-member" mode="synopsis">
        ...
        <xsl:if test="purpose">
            ...
            <xsl:call-template name="highlight-comment">
                <xsl:with-param name="text">
                    <xsl:text>//</xsl:text>
                    <xsl:apply-templates mode="purpose" select="purpose/*|purpose/text()" />
                </xsl:with-param>
            </xsl:call-template>
        </xsl:if>
    </xsl:template>
    ...
    <xsl:template match="enum" mode="synopsis">
        ...
        <!-- Spacing -->
        <xsl:if test="(not (local-name(preceding-sibling::*[position()=1])=local-name(.))
             and (position() &gt; 1)) or
        ...
        <xsl:if test="para or description or not ($boost.compact.enum=1)">
            <xsl:call-template name="reference-documentation">
                ...
                <xsl:with-param name="text">
                    ...
                    <xsl:if test="para">
                        <xsl:message>
                            <xsl:text>Warning: Use of 'para' elements in 'enum' element is
                                deprecated.&#10;Wrap them in a 'description' element.
                            </xsl:text>
                        </xsl:message>
                        <xsl:call-template name="print.warning.context" />
                        <xsl:apply-templates mode="annotation" select="para" />
                    ...
                    <xsl:if test="enumvalue/purpose | enumvalue/description">
                        <variablelist spacing="compact">
                            <xsl:apply-templates mode="reference" select="enumvalue" />
                        </variablelist>
                    </xsl:if>
                </xsl:with-param>
    ...
    <xsl:template name="type.enum.list.compact">
        ...
        <xsl:if test="not($pos &gt; count(enumvalue))">
            ...
            <xsl:if test="$value/default">
                <xsl:text>=</xsl:text>
                <xsl:apply-templates select="$value/default/*|$value/default/text()" />
            </xsl:if>
    ...
    <xsl:template match="enumvalue" mode="reference">
        <xsl:if test="purpose or description">
            <varlistentry>
                ...
                <listitem>
                    <xsl:apply-templates mode="comment" select="purpose|description" />
                </listitem>
            </varlistentry>
        </xsl:if>
    </xsl:template>
