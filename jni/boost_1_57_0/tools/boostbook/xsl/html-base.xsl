...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
    ...
    <xsl:param name="callout.graphics.path" select="concat($boost.graphics.root, 'callouts/')" />
    ...
    <xsl:template name="format.svn.revision">
        ...
        <xsl:variable name="text.noprefix" select="substring-after($text, ': ')" />
    ...
    <xsl:template match="copyright" mode="boost.footer">
        ...
        <xsl:call-template name="gentext.space" />
        <xsl:apply-templates mode="titlepage.mode" select="holder" />
    </xsl:template>

    <!-- Footer License -->
    <xsl:template match="legalnotice" mode="boost.footer">
        <xsl:apply-templates mode="titlepage.mode" select="para" />
    </xsl:template>
    ...
    <xsl:template name="user.footer.content">
        <table width="100%">
            <tr>
                <td align="left">
                    <xsl:variable name="revision-nodes" select="ancestor-or-self::*
                    ...
                    <xsl:if test="count($revision-nodes) &gt; 0">
                        ...
                        <xsl:if
                            test="string-length($revision-text) &gt; 0 and not($revision-text = '$Date$')">
                            <p>
                                <small>
                                    <xsl:text>Last revised:</xsl:text>
                                    <xsl:choose>
                                        <xsl:when
                                            test="not(contains($revision-text, '$Date: ')) and not(contains($revision-text, '$Date:: '))">
                ...
                <td align="right">
                    <div class="copyright-footer">
                        <xsl:apply-templates mode="boost.footer" select="ancestor::*/*/copyright" />
                        <xsl:apply-templates mode="boost.footer"
                            select="ancestor::*/*/legalnotice" />
                    </div>
                </td>
            </tr>
        </table>
    ...
    <xsl:template name="user.head.content">
        <xsl:if test="$boost.mathjax = 1">
            ...
            <xsl:if test="string($has-math) != ''">
                <script src="{$boost.mathjax.script}" type="text/javascript" />
            </xsl:if>
        </xsl:if>
    </xsl:template>
                ...
                <xsl:variable name="plain-content">
                    <xsl:choose>
                        ...
                        <xsl:when test="substring($content, 1, 1) = '$' and
                            ...
                            <xsl:value-of
                                select="substring($content, 2, string-length($content) - 2)" />
                        </xsl:when>
                        <xsl:otherwise>
                    ...
                    <xsl:attribute name="href">
                        <xsl:call-template name="href.target.relative">
                            <xsl:with-param name="target"
                                select="substring-before($stylesheets, ' ')" />
                        </xsl:call-template>
                    </xsl:attribute>
