...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
    ...
    <xsl:output doctype-public="-//OASIS//DTD DocBook XML V4.2//EN" doctype-system="http://www.oasis-open.org/docbook/xml/4.2/docbookx.dtd"
        method="xml" />
    ...
    <xsl:template match="header">
        <xsl:if test="*">
            <section>
                ...
                <xsl:apply-templates mode="annotation" select="para|section" />
                ...
                <xsl:if test="descendant::class|descendant::struct|descendant::union
                    ...
                    <xsl:call-template name="synopsis">
                        <xsl:with-param name="text">
                            <xsl:apply-templates mode="synopsis" select="namespace|class|struct|union
            ...
            <xsl:attribute name="url">
                <xsl:choose>
                    <xsl:when test="starts-with(@url, 'boost:/')">
                        <xsl:value-of
                            select="concat($boost.protocol.text, substring-after(@url, 'boost:'))" />
                    </xsl:when>
                    <xsl:when test="starts-with(@url, 'boost:')">
                        <xsl:value-of
                            select="concat($boost.protocol.text, '/', substring-after(@url, 'boost:'))" />
                    ...
                    <xsl:when
                        test="$boost.url.prefix != '' and not(contains(@url, ':') or starts-with(@url, '//'))">
    ...
    <xsl:template name="link-or-anchor">
        ...
        <xsl:choose>
            ...
            <xsl:otherwise>
                <xsl:message>
                    Error: XSL template 'link-or-anchor' called with invalid link-type '
    ...
    <xsl:template match="library">
        <xsl:if test="not(@html-only = 1) and
            ...
            <chapter>
                <xsl:copy-of
                    select="@*[not(contains(' last-revision name dirname html-only url ', concat(' ',local-name(),' ')))]" />
                <xsl:if test="not(@id)">
                    <xsl:attribute name="id">
                        <xsl:call-template name="generate.id" />
                    </xsl:attribute>
