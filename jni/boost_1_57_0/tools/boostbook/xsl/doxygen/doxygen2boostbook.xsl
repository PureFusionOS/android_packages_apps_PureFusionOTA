...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
    ...
    <xsl:param name="boost.doxygen.detail"><xsl:value-of select="$boost.doxygen.detailns" />::
    </xsl:param>
    ...
    <xsl:output indent="no" method="xml" standalone="yes" />
    ...
    <xsl:variable name="boost.doxygen.formuladir.fixed">
        <xsl:choose>
            <xsl:when
                test="substring(boost.doxygen.formuladir, string-length(boost.doxygen.formuladir) - 1) = '/'">
    ...
    <xsl:template match="doxygen">
        <library-reference>
            <xsl:if test="string($boost.doxygen.reftitle) != ''">
                ...
                <title>
                    <xsl:copy-of select="$boost.doxygen.reftitle" />
                </title>
            </xsl:if>
        ...
        <xsl:choose>
            ...
            <xsl:when test="@kind='namespace'">
                <xsl:call-template name="namespace">
                    <xsl:with-param name="with-namespace-refs" select="$with-namespace-refs" />
    ...
    <xsl:template name="class">
        ...
        <xsl:if test="string(location/attribute::file)=$in-file">
            ...
            <xsl:element name="{$actual-class-key}">
                ...
                <xsl:apply-templates mode="template" select="templateparamlist" />
                ...
                <xsl:apply-templates mode="inherit" select="basecompoundref" />

                <xsl:apply-templates mode="passthrough" select="briefdescription" />
                <xsl:apply-templates mode="passthrough" select="detaileddescription" />
                <xsl:apply-templates mode="passthrough" select="inbodydescription" />
                <xsl:apply-templates>
                    <xsl:with-param name="in-file" select="$in-file" />
                </xsl:apply-templates>
            </xsl:element>
                    ...
                    <xsl:if test="initializer">
                        <default>
                            <xsl:apply-templates mode="passthrough"
                                select="initializer/*|initializer/text()" />
                        </default>
                    </xsl:if>
    ...
    <xsl:template name="doxygen.include.header.rec">
        ...
        <xsl:choose>
            <xsl:when test="contains($header-list, ' ')">
                <xsl:variable name="header" select="substring-before($header-list, ' ')" />
        ...
        <xsl:if test="$include-header='yes'">
            <header>
                ...
                <xsl:if test="briefdescription/*|detaileddescription/*|inbodydescription/*">
                    <xsl:apply-templates mode="passthrough" select="briefdescription/*" />
                    <xsl:apply-templates mode="passthrough" select="detaileddescription/*" />
                    <xsl:apply-templates mode="passthrough" select="inbdoydescription/*" />
                </xsl:if>

                <xsl:apply-templates mode="toplevel">
                    <xsl:with-param name="with-namespace-refs" select="innernamespace" />
    ...
    <xsl:template match="sectiondef" mode="toplevel">
        ...
        <xsl:apply-templates mode="toplevel" select="memberdef[generate-id() =
                    ...
                    <xsl:for-each select="param">
                        ...
                        <macro-parameter>
                            ...
                            <xsl:if test="$description">
                                <description>
                                    <xsl:apply-templates mode="passthrough" select="$description" />
                                </description>
                            </xsl:if>
                        </macro-parameter>
                    </xsl:for-each>
            ...
            <xsl:otherwise>
                <xsl:call-template name="kind-error-message">
                    <xsl:with-param name="message"
                        select="'Cannot handle toplevel memberdef element'" />
                </xsl:call-template>
            </xsl:otherwise>
        </xsl:choose>
    </xsl:template>
    ...
    <xsl:template match="param" mode="template">
            ...
            <xsl:when test="string(type)='class' or string(type)='typename'">
                ...
                <template-type-parameter>
                    ...
                    <xsl:if test="defval">
                        <default>
                            <xsl:apply-templates mode="passthrough"
                                select="defval/*|defval/text()" />
                        </default>
                    </xsl:if>
                    <xsl:for-each
                        select="../../detaileddescription//parameterlist[@kind='templateparam']/parameteritem">
        ...
        (starts-with(string(type), 'class ') or starts-with(string(type), 'typename '))">
                <template-type-parameter>
                    <xsl:variable name="name">
                        <xsl:value-of
                            select="normalize-space(substring-after(string(type), ' '))" />
                    </xsl:variable>
                    <xsl:attribute name="name">
                        <xsl:value-of select="$name" />
                    </xsl:attribute>
        ...
        <xsl:choose>
            ...
            <xsl:when test="@kind='public-func'">
                ...
                <xsl:variable name="num-internal-only">
                    <xsl:value-of select="count($members[contains(detaileddescription/para,
                                  'INTERNAL ONLY')])" />
                </xsl:variable>
            ...
            <xsl:when test="@kind='friend'">
                <xsl:if
                    test="./memberdef/detaileddescription/para or ./memberdef/briefdescription/para">
                        ...
                        <xsl:variable name="in-class">
                            <xsl:choose>
                                <xsl:when test="contains($in-class-full, '&lt;')">
                                    <xsl:value-of
                                        select="substring-before($in-class-full, '&lt;')" />
                                </xsl:when>
                                <xsl:otherwise>
    ...
    <xsl:template match="param" mode="function">
        <parameter>
            ...
            <xsl:apply-templates
                ...
                select="../*[self::detaileddescription or self::inbodydescription]//parameterlist[attribute::kind='param']/*">
                <xsl:with-param name="name">
                    <xsl:value-of select="$name" />
                </xsl:with-param>
            </xsl:apply-templates>
    ...
    <xsl:template match="parameteritem" mode="parameter.description">
        ...
        <xsl:if test="string(parameternamelist/parametername) = $name">
            <description>
                <xsl:apply-templates mode="passthrough" select="parameterdescription/para" />
            </description>
        </xsl:if>
    </xsl:template>
    ...
    <xsl:template match="parameterdescription" mode="parameter.description">
        ...
        <xsl:if test="string($name-node/text()) = $name">
            <description>
                <xsl:apply-templates mode="passthrough" select="para" />
            </description>
        </xsl:if>
    </xsl:template>
    ...
    <xsl:template name="function.attributes">
        ...
        <xsl:variable name="noexcept">
            <xsl:choose>
                ...
                <xsl:when
                    test="contains($extra-qualifiers, ' noexcept ') or contains($extra-qualifiers, ' BOOST_NOEXCEPT ')">
          ...
          999 * not(contains($prefix3, &quot;'&quot;)))" />
            ...
            <xsl:choose>
                    ...
                    <xsl:variable name="text2">
                        <xsl:call-template name="strip-brackets">
                            <xsl:with-param name="text"
                                select="substring($text, $prefix-length + 2)" />
                        </xsl:call-template>
                    </xsl:variable>
                    <xsl:call-template name="strip-brackets">
                ...
                <xsl:when test="$char=&quot;'&quot;">
                    ...
                    <xsl:call-template name="strip-brackets">
                        <xsl:with-param name="text"
                            select="substring-after(substring($text, $prefix-length + 2), &quot;'&quot;)" />
                    </xsl:call-template>
                </xsl:when>
                <xsl:when test="$char='&quot;'">
                    <!-- Not bothering with escapes, because this is crazy enough as it is -->
    ...
    <xsl:template name="function.children">
        ...
        <xsl:if test="not($is-overloaded)">
            ...
            <xsl:apply-templates mode="function" select="param" />
        </xsl:if>
        ...
        <xsl:if
            test="*[self::detaileddescription or self::inbodydescription]/para/parameterlist[@kind='exception']">
            <throws>
                <xsl:apply-templates
                    mode="function-clauses"
    ...
    <xsl:template name="function">
        ...
        <xsl:variable name="firstpara" select="normalize-space(detaileddescription/para[1])" />
        <xsl:if test="string(location/attribute::file)=$in-file
                  and 
            ...
            <xsl:variable name="has-overload">
                ...
                <xsl:if test="not(normalize-space($next-node/detaileddescription/para[1])
        ...
        <xsl:choose>
            ...
            <xsl:otherwise>
                <signature>
                    ...
                    <xsl:apply-templates mode="template" select="$node/templateparamlist" />
                    <xsl:apply-templates mode="function" select="$node/param" />
                </signature>
    ...
    <xsl:template match="ref" mode="passthrough">
        <xsl:variable name="as-class"
            select="key('compounds-by-id', @refid)[@kind='class' or @kind='struct']" />
        <xsl:choose>
            ...
            <simpara>
                <xsl:choose>
                    ...
                    <xsl:otherwise>
                        ...
                        <xsl:text></xsl:text>
                        <xsl:apply-templates mode="passthrough" select=".//parameterdescription/para/text()
    ...
    <xsl:template name="table-entry">
        <xsl:choose>
            <xsl:when test="para/center">
                <xsl:apply-templates mode="passthrough" select="para/center/*|para/center/text()" />
            </xsl:when>
            <xsl:otherwise>
                <xsl:apply-templates mode="passthrough" select="para/*|para/text()" />
            </xsl:otherwise>
        </xsl:choose>
    </xsl:template>
    ...
    <xsl:template match="para" mode="purpose">
        <xsl:apply-templates mode="passthrough" select="*|text()" />
    </xsl:template>
                        ...
                        <imageobject role="html">
                            <imagedata align="center" format="PNG">
                                <xsl:attribute name="fileref">
                                    <xsl:value-of
                                        select="concat(concat(concat($boost.doxygen.formuladir.fixed, 'form_'), @id), '.png')" />
                                </xsl:attribute>
                            </imagedata>
                        </imageobject>
                        <textobject role="tex">
