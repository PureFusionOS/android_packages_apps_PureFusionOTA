...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
    ...
    <xsl:template match="/">
        <xsl:if test="$boost.root">
            ...
            <xsl:apply-templates mode="Jamfile" select="//testsuite" />

            <!-- Output any source files that are written in XML -->
            <xsl:apply-templates mode="testsuite.generate" select="//source" />
        </xsl:if>
    ...
    <xsl:template match="testsuite" mode="Jamfile">
        ...
        <xsl:variable name="filename" select="concat($boost.root,'/libs/',ancestor::library/attribute::dirname,
                     '/test/Jamfile')" />
        <xsl:call-template name="write.text.chunk">
            ...
            <xsl:with-param name="content">
                ...
                test-suite
                <xsl:value-of select="ancestor::library/attribute::dirname" />
                :
    ...
    <xsl:template
        match="compile-test|link-test|run-test|compile-fail-test|link-fail-test|run-fail-test"
        mode="Jamfile">
        <xsl:variable name="fullname" select="concat(substring-before(local-name(.), '-test'), ' libs/',
                     ancestor::library/attribute::dirname, '/test/', 
                     @filename)" />
        <xsl:text>[</xsl:text>
        <xsl:value-of select="$fullname" />
        <xsl:apply-templates mode="Jamfile" select="lib" />
        <xsl:text>:</xsl:text>
        <!-- ... -->
        <xsl:text>:</xsl:text>
        <!-- ... -->
        <xsl:text>:</xsl:text>
        <xsl:apply-templates mode="Jamfile" select="requirement" />
        <!-- ... -->
        <xsl:text>:</xsl:text>
        <xsl:if test="@name">
    ...
    <xsl:template name="source.print.header">
        ...
        <!-- Copyrights -->
        <xsl:apply-templates mode="testsuite.generate"
            select="ancestor::library/libraryinfo/copyright">
            <xsl:with-param name="prefix" select="$prefix" />
        </xsl:apply-templates>

        <!-- Legal notice -->
        <xsl:apply-templates mode="testsuite.generate"
            select="ancestor::library/libraryinfo/legalnotice">
            <xsl:with-param name="prefix" select="$prefix" />
        </xsl:apply-templates>
    ...
    <xsl:template name="source.print.legalnotice">
        ...
        <xsl:if test="contains($text, ' ') or string-length($text) &gt; 0">
            ...
            <xsl:choose>
                <xsl:when test="$column + string-length($word) &gt; $boost.source.columns">
                    <xsl:text>&#10;</xsl:text>
                    <xsl:value-of select="concat($prefix, ' ')" />
                    <xsl:call-template name="source.print.legalnotice">
                        <xsl:with-param name="prefix" select="$prefix" />
                ...
                <xsl:otherwise>
                    ...
                    <xsl:call-template name="source.print.legalnotice">
                        ...
                        <xsl:with-param name="column" select="$column + string-length($word) + 1" />
                    </xsl:call-template>
                </xsl:otherwise>
            </xsl:choose>
        </xsl:if>
