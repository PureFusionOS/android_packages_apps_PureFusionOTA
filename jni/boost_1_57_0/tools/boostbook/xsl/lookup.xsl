...
<xsl:stylesheet xmlns:exsl="http://exslt.org/common"
    ...
    <xsl:template name="generate.id">
        ...
        <xsl:apply-templates mode="generate.id" select="$node" />
    </xsl:template>
    ...
    <xsl:template name="fully-qualified-id">
        ...
        <xsl:apply-templates mode="fully-qualified-name" select="$node">
    ...
    <xsl:template name="build-fully-qualified-name">
        ...
        <xsl:variable name="ancestors" select="ancestor::namespace|
        ...
        <xsl:for-each select="$ancestors">
            <xsl:apply-templates mode="fast-print-id-part" select=".">
                <xsl:with-param name="is.id" select="$is.id" />
            </xsl:apply-templates>
            <xsl:choose>
                <xsl:when test="$is.id">
    ...
    <xsl:variable name="elements-with-ids">
        <xsl:apply-templates
            mode="preprocess-ids"
            select="namespace|class|struct|union|class-specialization|struct-specialization|union-specialization" />
    </xsl:variable>
    ...
    <xsl:template name="print-id-part">
        <xsl:apply-templates mode="print-id-part" select="." />
    </xsl:template>
    ...
    <xsl:template match="*" mode="fast-print-id-part">
        ...
        <xsl:choose>
            <xsl:when test="not($is.id)">
                <xsl:apply-templates mode="print-name" select="." />
            </xsl:when>
    ...
    <xsl:template match="*" mode="print-id-part">
        ...
        <xsl:variable name="unique-name">
            <xsl:apply-templates mode="unique.name" select="." />
        </xsl:variable>

        <xsl:choose>
            <xsl:when test="$is.id and (
          string-length($part) &gt; $boost.max.id.part.length or
          $unique-name = 0 or
          translate($part, '.&lt;&gt;;\:*?&quot;| ', '') != $part
        )">
                <xsl:variable name="normalized"
                    select="translate(normalize-space(translate($part, '.&lt;&gt;;\:*?&quot;|_', '            ')), ' ', '_')" />
    ...
    <xsl:template match="struct-specialization|class-specialization|union-specialization"
        mode="print-name">
        <xsl:value-of select="@name" />
        <xsl:text>&lt;</xsl:text>
        <xsl:apply-templates mode="print-name" select="specialization/template-arg" />
        <xsl:text>&gt;</xsl:text>
    </xsl:template>
    ...
    <xsl:template name="find-nodes-matching-name">
        ...
        <xsl:apply-templates mode="generate-cxx-links" select="$nodes">
            <xsl:with-param name="name" select="$name" />
    ...
    <xsl:template match="*" mode="generate-cxx-links">
        ...
        <xsl:if test="string-length($node-name) &gt;= string-length($name) and
            ...
            <xsl:variable name="qualifiers" select="substring($node-name, 1, $leading-chars)" />
