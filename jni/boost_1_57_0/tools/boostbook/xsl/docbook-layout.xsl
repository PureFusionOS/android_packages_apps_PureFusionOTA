...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
    ...
    <xsl:import href="http://docbook.sourceforge.net/release/xsl/current/html/formal.xsl" />
    ...
    <xsl:template match="table|informaltable">
        <xsl:choose>
            ...
            <xsl:when test="self::informaltable and tgroup|mediaobject|graphic">
            ...
            <xsl:otherwise>
                <table class="table">
                    <xsl:copy-of select="@*[not(local-name(.)='border')]" />
    ...
    <xsl:template name="tgroup" match="tgroup">
        ...
        <table class="table">
            ...
            <xsl:if test=".//footnote">
                <tbody class="footnotes">
                    <tr>
                        <td colspan="{@cols}">
                            <xsl:apply-templates mode="table.footnote.mode" select=".//footnote" />
                        </td>
                    </tr>
                </tbody>
            </xsl:if>
    ...
    <xsl:template match="table" mode="title.markup">
        ...
        <xsl:apply-templates mode="title.markup" select="(title|caption)[1]">
    ...
    <xsl:template match="section[@label-style='no-parent']" mode="label.markup">
        <xsl:choose>
            ...
            <xsl:when test="$label != 0">
                ...
                <xsl:number count="section" format="{$format}" />
            </xsl:when>
        </xsl:choose>
    </xsl:template>
