      ...
      -->
            ...
            <xsl:variable name="notations">
                <xsl:for-each select="notation">
                    ...
                    @@=@@<xsl:value-of select="normalize-space(@variables)" />@@)@@
                </xsl:for-each>
            </xsl:variable>
                                    ...
                                    <para>
                                        <xsl:call-template name="unparse-operator-definition">
                                            ...
                                            <xsl:with-param name="operator_nodeset"
                                                select="key('concepts', @concept)/models-sentence/node()" />
                                            <xsl:with-param name="definition_list"
            ...
            <xsl:variable name="see-also-list-0"
                select="concept-ref | see-also | refines | refines-when-mutable | models-as-first-arg | models | models-when-mutable" />
            <xsl:variable name="see-also-list-1"
                select="$see-also-list-0[string(@name | @concept) != string(../@name)]" />
            <xsl:variable name="see-also-list"
                select="$see-also-list-1[not(string(@name|@concept) = (preceding::*/@name | preceding::*/@concept | ancestor::*/@name | ancestor::*/@concept))]" />
            <xsl:if test="$see-also-list">
                <refsect1>
                    <title>See also</title>
                    <itemizedlist>
                        <xsl:for-each select="$see-also-list">
                            <xsl:sort data-type="text" select="string(@name|@concept)" />
                            <listitem>
                                <para>
                                    <xsl:call-template name="concept.link">
        ...
        <xsl:choose>
            ...
            <xsl:when test="name($constraint)='convertible-to'">
                <xsl:choose>
                    ...
                    <xsl:when test="not($type-expr-mode) and not($capitalize)">convertible to
                    </xsl:when>
                </xsl:choose>
                <type>
                    <xsl:call-template name="unparse-cpp">
            ...
            <xsl:when test="name($constraint)='derived-from'">
                <xsl:choose>
                    ...
                    <xsl:when test="not($type-expr-mode) and not($capitalize)">derived from
                    </xsl:when>
                </xsl:choose>
                <type>
                    <xsl:call-template name="unparse-cpp">
            ...
            <xsl:when test="name($constraint)='assignable-to'">
                <xsl:choose>
                    ...
                    <xsl:when test="not($type-expr-mode) and not($capitalize)">assignable to
                    </xsl:when>
                </xsl:choose>
                <type>
                    <xsl:call-template name="unparse-cpp">
        ...
        <xsl:choose>
            <xsl:when test="$typedefs">
                ...
                <xsl:call-template name="make-definition-list">
                    ...
                    <xsl:with-param name="definition_list"
                        select="concat($definition_list, ' @(@', $typedefs[1]/@name, '=', $new_type_definition, '@)@')" />
                </xsl:call-template>
                ...
                <xsl:choose>
                    <xsl:when test="$boost.concept.layout='sgi'">
                        <informaltable>
                            <tgroup cols="2">
                                <tbody>
                                    <xsl:apply-templates mode="sgi" select="associated-type">
                                        <xsl:with-param name="definition_list"
                                            select="$definition_list" />
                                    </xsl:apply-templates>
                                </tbody>
                    ...
                    <xsl:when test="$boost.concept.layout='austern'">
                        <itemizedlist>
                            <xsl:apply-templates mode="austern" select="associated-type">
                                <xsl:with-param name="definition_list" select="$definition_list" />
                            </xsl:apply-templates>
                        </itemizedlist>
                    </xsl:when>
            ...
            <variablelist>
                ...
                <xsl:for-each select="notation">
                    <xsl:variable name="notation_name">
                        <xsl:call-template name="comma-list">
                            <xsl:with-param name="list" select="normalize-space(@variables)" />
                        </xsl:call-template>
                    </xsl:variable>
                    ...
                    <varlistentry>
                        ...
                        <listitem>
                            <simpara>
                                <xsl:variable name="output-plural"
                                    select="substring-before(normalize-space(@variables),' ')!=''" />
                                <xsl:if test="name(*[1])='sample-value'">Object<xsl:if
                                    test="$output-plural">s
                                </xsl:if> of type
                                </xsl:if>
                                <xsl:variable name="typeref-to-print"
                                    select="*[name()!='sample-value'] | sample-value/*[name()!='sample-value']" />
                                <xsl:call-template name="unparse-cpp">
        ...
        <xsl:choose>
            <xsl:when test="contains($normalized, ' ')">
                ...
                <xsl:call-template name="remove-whitespace">
                    <xsl:with-param name="text" select="substring-after($normalized, ' ')" />
                </xsl:call-template>
            </xsl:when>
            <xsl:otherwise>
