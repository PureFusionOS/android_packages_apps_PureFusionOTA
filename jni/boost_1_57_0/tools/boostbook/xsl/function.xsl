...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
    ...
    <xsl:template name="function">
        ...
        <xsl:variable name="type">
            ...
            <xsl:choose>
                ...
                <xsl:otherwise>
                    <xsl:apply-templates mode="annotation" select="type" />
                    <xsl:text></xsl:text>
                </xsl:otherwise>
            </xsl:choose>
        </xsl:variable>
        ...
        <xsl:choose>
            ...
            <xsl:when test="not($end-column &gt; $max-columns)">
                ...
                <xsl:call-template name="function-parameters">
                    <xsl:with-param name="include-names" select="$include-names" />
                    <xsl:with-param name="indentation" select="$indentation + $template-length + string-length($type)
            ...
            <xsl:otherwise>
                ...
                <xsl:call-template name="function-parameters">
                    <xsl:with-param name="include-names" select="$include-names" />
                    <xsl:with-param name="indentation" select="$indentation + $type-length
                    + string-length($function-name) + 1" />
                    <xsl:with-param name="final" select="true()" />
                </xsl:call-template>
                <xsl:call-template name="highlight-text">
                            ...
                            <xsl:when test="$final">
                                <xsl:call-template name="highlight-text">
                                    <xsl:with-param name="text"
                                        select="concat(' = ', $parameter/@default)" />
                                </xsl:call-template>
                            </xsl:when>
                            <xsl:otherwise>
                                <xsl:text>=</xsl:text>
    ...
    <xsl:template match="function|method" mode="synopsis">
        ...
        <xsl:variable name="compact" select="not (para|description|requires|effects|postconditions|returns|
    ...
    <xsl:template match="free-function-group" mode="header-synopsis">
        ...
        <xsl:param name="indentation" />
        <xsl:apply-templates mode="synopsis" select="function|overloaded-function">
            <xsl:with-param name="indentation" select="$indentation" />
        </xsl:apply-templates>
    </xsl:template>
    ...
    <xsl:template name="construct-copy-destruct-synopsis">
        ...
        <xsl:if test="constructor|copy-assignment|destructor">
            ...
            <xsl:apply-templates mode="synopsis" select="constructor">
                <xsl:with-param name="indentation" select="$indentation" />
            </xsl:apply-templates>
            <xsl:apply-templates mode="synopsis" select="copy-assignment">
                <xsl:with-param name="indentation" select="$indentation" />
            </xsl:apply-templates>
            <xsl:apply-templates mode="synopsis" select="destructor">
                <xsl:with-param name="indentation" select="$indentation" />
            </xsl:apply-templates>
        </xsl:if>
    </xsl:template>
    ...
    <xsl:template name="construct-copy-destruct-reference">
        <xsl:if test="constructor|copy-assignment|destructor">
            <xsl:call-template name="member-documentation">
                ...
                <xsl:with-param name="text">
                    <orderedlist>
                        <xsl:apply-templates mode="reference" select="constructor" />
                        <xsl:apply-templates mode="reference" select="copy-assignment" />
                        <xsl:apply-templates mode="reference" select="destructor" />
                    </orderedlist>
                </xsl:with-param>
            </xsl:call-template>
        </xsl:if>
        ...
        <xsl:if test="parameter/description|signature/parameter/description|
            ...
            <variablelist spacing="compact">
                ...
                <xsl:for-each select="requires|effects|postconditions|returns|throws|complexity|
                      notes|rationale">
                    <varlistentry>
                        <term><xsl:call-template name="function.requirement.name" />:
                        </term>
                        <listitem>
                            <xsl:apply-templates mode="annotation" select="./*|./text()" />
                        </listitem>
                    </varlistentry>
                </xsl:for-each>
    ...
    <xsl:template match="method-group" mode="synopsis">
        ...
        <xsl:if test="count(child::*) &gt; 0">
            ...
            <xsl:apply-templates mode="synopsis" select="method|overloaded-method">
                <xsl:with-param name="indentation" select="$indentation" />
            </xsl:apply-templates>
        </xsl:if>
    </xsl:template>
    ...
    <xsl:template match="method-group" mode="reference">
        <xsl:if test="count(child::*) &gt; 0">
            <xsl:call-template name="member-documentation">
                ...
                <xsl:with-param name="text">
                    <orderedlist>
                        <xsl:apply-templates mode="reference" select="method|overloaded-method" />
                    </orderedlist>
                </xsl:with-param>
            </xsl:call-template>
        </xsl:if>
    ...
    <xsl:template match="free-function-group" mode="reference">
        ...
        <xsl:call-template name="member-documentation">
            ...
            <xsl:with-param name="text">
                <orderedlist>
                    <xsl:apply-templates mode="reference" select="function|overloaded-function" />
                </orderedlist>
            </xsl:with-param>
        </xsl:call-template>
    </xsl:template>
