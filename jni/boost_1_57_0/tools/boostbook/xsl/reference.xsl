...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
    ...
    <xsl:template name="namespace-synopsis">
        ...
        <xsl:apply-templates mode="synopsis" select="class|class-specialization|
                                 struct|struct-specialization|
                                 union|union-specialization|
                                 typedef|enum|data-member">
    ...
    <xsl:template match="namespace" mode="reference">
        <xsl:apply-templates mode="reference" select="namespace|free-function-group">
            ...
            <xsl:otherwise>
                ...
                <xsl:call-template name="wrap-comment">
                    ...
                    <xsl:with-param name="text"
                        select="normalize-space(substring($text, $size + 1))" />
                </xsl:call-template>
            </xsl:otherwise>
        </xsl:choose>
    </xsl:template>
    ...
    <xsl:template match="purpose" mode="comment">
        ...
        <xsl:apply-templates mode="comment">
            <xsl:with-param name="wrap" select="$wrap and count(text()|*) = 1" />
            <xsl:with-param name="prefix" select="$prefix" />
        </xsl:apply-templates>
    </xsl:template>
    ...
    <xsl:template match="simpara|para" mode="comment">
        <xsl:param name="wrap" select="false()" />
        <xsl:param name="prefix" />
        <xsl:apply-templates mode="comment" select="text()|*">
            <xsl:with-param name="wrap" select="$wrap and count(text()|*) = 1" />
            <xsl:with-param name="prefix" select="$prefix" />
        </xsl:apply-templates>
    </xsl:template>
