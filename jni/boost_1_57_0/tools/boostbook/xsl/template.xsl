...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
    ...
    <xsl:template name="template.synopsis.parameters">
        ...
        <xsl:param name="parameters"
            select="template-type-parameter|template-varargs|template-nontype-parameter" />
                            ...
                            <xsl:with-param name="prefix">
                                <xsl:call-template name="indent">
                                    <xsl:with-param name="indentation"
                                        select="$indentation + string-length($param-text)" />
                                </xsl:call-template>
                                <xsl:if test="$rest">
                                    <xsl:text></xsl:text>
                                </xsl:if>
    ...
    <xsl:template name="template.parameter">
        ...
        <xsl:param name="highlight" select="true()" />
        <xsl:apply-templates mode="print.parameter" select="$parameter">
            <xsl:with-param name="parameter" select="$parameter" />
        ...
        <xsl:choose>
            <xsl:when test="$highlight">
                <xsl:call-template name="concept.link">
                    <xsl:with-param name="name" select="translate($name, '0123456789', '')" />
    ...
    <xsl:template match="template-type-parameter" mode="print.parameter">
        ...
        <xsl:variable name="def">
            <xsl:choose>
                <xsl:when test="$parameter/@default">
                    <xsl:message>
                        <xsl:text>Warning: 'default' attribute of template parameter element is
                            deprecated. Use 'default' element.
                        </xsl:text>
        ...
        <xsl:if test="not($def='')">
            ...
            <!-- If this is the last parameter, add an extra space to
                 avoid printing >> -->
            <xsl:if test="$is-last and (substring($def, string-length($def))='&gt;')">
                <xsl:text></xsl:text>
            </xsl:if>
        </xsl:if>
    </xsl:template>
    ...
    <xsl:template match="template-nontype-parameter" mode="print.parameter">
        ...
        <xsl:choose>
            <xsl:when test="$highlight">
                <xsl:call-template name="source-highlight">
                    <xsl:with-param name="text">
                        <xsl:apply-templates select="$parameter/type/*|$parameter/type/text()" />
                    </xsl:with-param>
                </xsl:call-template>
            </xsl:when>
            <xsl:otherwise>
