...
sub age_info {
    ...
    if ($age_days <= 1) {$age_html = $age_html . "today";}
        ...
        $age_html = $age_html . $days . " days";
    }
    elsif ($age_days < 7 * 8) {
        my $weeks = int $age_days / 7;
        $age_html = $age_html . $weeks . " weeks";
    }
    else {
        my $months = int $age_days / 28;
        $age_html = $age_html . $months . " months";
    }
...
foreach $l (sort {lc($a) cmp lc($b)} @logs) {
    ...
    if ($fh->open("<$logdir/$log")) {
        ...
        for my $comp (1 .. (scalar @compiler)) {
            ...
            if (@lines > 2) {$compiler[$comp - 1] = join(' ',
                @lines[0 .. (scalar @lines) - 2]) . "<br>" . $lines[(scalar @lines) - 1];}
        }
        print
            "<tr>\n",
            ...
            "<td valign=\"top\" ", $bgcolor[$row], ">",
            result_info_pass("#000000", $pass[0], $warn[0], $fail[0], $missing[0]), "</td>\n",
            "<td valign=\"top\" ", $bgcolor[$row], ">",
            result_info_fail("#FF0000", $pass[0], $warn[0], $fail[0], $missing[0]), "</td>\n",
            "</tr>\n";
        $row = ($row + 1) % 2;
