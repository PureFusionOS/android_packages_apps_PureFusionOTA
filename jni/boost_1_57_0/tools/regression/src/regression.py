...
class runner:
    ...
    def command_test_boost_build(self):
        ...
        for t in toolsets:
            ...
            try:
                ...
                utils.system(
                    ['"%s" test_all.py --default-bjam --xml %s > %s' % (sys.executable, t, fn)])
            finally:
                os.chdir(cd)
    ...
    def compress_file(self, file_path, archive_path):
        ...
        try:
            ...
            try:
                ...
                utils.log(
                    'Could not find \'zip_cmd\' module in the script directory (%s).' % script_dir)
    ...
    def get_tarball(self, *args):
        ...
        if hasattr(self, 'local') and self.local is not None:
            ...
            tarball_path = self.download_tarball(self.boost_tarball_name(),
                                                 self.boost_tarball_url())
        if not tarball_path:
