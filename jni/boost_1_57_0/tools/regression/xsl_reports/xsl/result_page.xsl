...
<xsl:stylesheet xmlns:exsl="http://exslt.org/common"
    ...
    <xsl:variable name="not_ordered_toolsets"
        select="//test-log[ generate-id(.) = generate-id( key('toolset_key',@toolset)[1] ) and @toolset != '' ]/@toolset" />
    ...
    <xsl:template name="insert_toolsets_row">
        ...
        <tr valign="middle">
            ...
            <exsl:document encoding="utf-8" href="debug2.xml" indent="yes" method="xml">
                <debug>
            ...
            <xsl:for-each select="$toolsets/toolset">
                ...
                <xsl:variable name="toolset_notes_fragment">
                    <xsl:for-each select="$all_library_notes">
                        <xsl:if
                            test="../@toolset=$toolset or ( ../toolset/@name=$toolset or ../toolset/@name = '*' )">
                            <note index="{position()}" />
                        </xsl:if>
                    </xsl:for-each>
                </xsl:variable>

                <xsl:variable name="toolset_notes"
                    select="exsl:node-set( $toolset_notes_fragment )/*" />

                <td class="{$class}">
                    <xsl:value-of select="meta:toolset_name( $toolset )" />
                    <xsl:if test="count( $toolset_notes ) > 0">
                        <span class="super">
                            <xsl:for-each select="$toolset_notes">
                                <xsl:variable name="note_index" select="@index" />
                                <xsl:if test="generate-id( . ) != generate-id( $toolset_notes[1] )">
                                    ,
                                </xsl:if>
    ...
    <xsl:template name="test_type_col">
        <td class="test-type">
            <a class="legend-link" href="../../../status/compiler_status.html#Understanding">
    ...
    <xsl:template match="/">
        ...
        <exsl:document doctype-public="-//W3C//DTD XHTML 1.0 Strict//EN" encoding="utf-8"
            ...
            <html>
                ...
                <body>
                    ...
                    <img align="right" alt="Boost logo" border="0" height="86" src="../../../boost.png"
                        width="277"></img>
                        ...
                        <div>
                            ...
                            <xsl:choose>
                                <xsl:when test="$mode='user'">
                                    The purpose of this report is to help a user to find out whether
                                    a particular library
                                    works on the particular compiler(s). For CVS "health report",
                                    see
                                    <a href="../{$alternate_mode}/index.html" target="_top">
                                        developer summary</a>.
                                </xsl:when>
                                <xsl:when test="$mode='developer'">
                                    Provides Boost developers with visual indication of the CVS
                                    "health". For user-level
                                    report, see <a href="../{$alternate_mode}/index.html"
                                    target="_top">user summary</a>.
                                </xsl:when>
                            </xsl:choose>
                        </div>
                    </div>
                ...
                <html>
                    ...
                    <body class="{$mode}-toc">
                        <div class="toc-header-entry">
                            <a class="toc-entry" href="index.html" target="_top">Report info</a>
                        </div>
                        <div class="toc-header-entry">
                            <a class="toc-entry" href="summary.html" target="_top">Summary</a>
                        </div>
                        ...
                        <xsl:for-each select="$libraries">
                            ...
                            <div class="toc-entry">
                                <a class="toc-entry" href="{$library_page}.html" target="_top">
                                    <xsl:value-of select="." />
                                </a>
                            </div>
                        </xsl:for-each>
        ...
        <xsl:for-each select="$libraries">
            ...
            <xsl:variable name="library_results"
                select="concat( meta:encode_path( $library ), '_.html' )" />
            <xsl:variable name="library_page"
                select="concat( meta:encode_path( $library ), '.html' )" />
            ...
            <exsl:document doctype-public="-//W3C//DTD XHTML 1.0 Strict//EN" encoding="utf-8"
                ...
                <html>
                    ...
                    <body>
                        ...
                        <h1 class="page-title">
                            <a name="{$library}" class="hover-link" href="../../../libs/{$library}">
                        ...
                        <xsl:variable name="library_marks"
                            select="$explicit_markup//library[ @name = $library ]/mark-unusable[ toolset/@name = $not_ordered_toolsets ]" />

                        <table border="0" cellpadding="0" cellspacing="0" class="library-table"
                            summary="library results">
                            ...
                            <tbody>
                                ...
                                <xsl:variable name="lib_corner_case_tests_markup"
                                    select="$explicit_markup//library[ @name = $library ]/test[ @corner-case='yes' ]" />
                                ...
                                <xsl:variable name="lib_corner_case_tests"
                                    select="meta:order_tests_by_name( $lib_unique_test_names[ @test-name = $lib_corner_case_tests_markup/@name ] ) " />

                                <!-- general tests section -->

                                <xsl:call-template name="insert_test_section">
                                    <xsl:with-param name="library" select="$library" />
                                    <xsl:with-param name="section_tests"
                                        select="$lib_general_tests" />
                                ...
                                <xsl:if test="count( $lib_corner_case_tests ) > 0">
                                    <tr>
                                        <td align="center"
                                            class="library-corner-case-header"
                                            colspan="{count($ordered_toolsets/toolset) + 3 }">Corner-case tests
                                        </td>
                                    </tr>
                                    ...
                                    <xsl:call-template name="insert_test_section">
                                        <xsl:with-param name="library" select="$library" />
                                        <xsl:with-param name="section_tests"
                                            select="$lib_corner_case_tests" />
                                        <xsl:with-param name="lib_tests" select="$lib_tests" />
                                        <xsl:with-param name="toolsets"
                                            select="$ordered_toolsets" />
                                    </xsl:call-template>
                                        ...
                                        <td>
                                            ...
                                            <xsl:call-template name="show_note">
                                                ...
                                                <xsl:with-param name="reference"
                                                    select="$explicit_markup//note[ $refid = @id ]" />
                                            </xsl:call-template>
                                        </td>
                                    </tr>
                                </xsl:for-each>
    ...
    <xsl:template name="insert_cell_developer">
        ...
        <xsl:variable name="is_new">
            <xsl:if
                test="$test_log/@is-new = 'yes' and $test_log/@status = 'unexpected' and $test_log/@result != 'success'">
        ...
        <xsl:variable name="class">
            <xsl:choose>
                ...
                <xsl:when
                    test="meta:is_unusable( $explicit_markup, $test_log/@library, $test_log/@toolset )">
                    <xsl:text>library-unusable</xsl:text>
                </xsl:when>
                <xsl:otherwise>
                    <xsl:value-of
                        select="concat( 'library-', $test_log/@result, '-', $test_log/@status, $is_new )" />
                </xsl:otherwise>
            </xsl:choose>
        </xsl:variable>
    ...
    <xsl:template name="insert_test_line">
        ...
        <xsl:variable name="test_header">
            <td class="test-name">
                <a class="test-link" href="../../../{$test_program}">
        ...
        <tr class="library-row{$line_mod}">
            ...
            <xsl:for-each select="$toolsets/toolset">
                ...
                <xsl:variable name="test_result_for_toolset"
                    select="$test_results[ @toolset = $toolset ]" />

                <xsl:variable name="log_file">
                    <xsl:choose>
                        <xsl:when
                            test="meta:show_output( $explicit_markup, $test_result_for_toolset )">
                            <xsl:value-of
                                select="meta:output_file_path( $test_result_for_toolset/@target-directory )" />
                        </xsl:when>
                        <xsl:otherwise>
                            <xsl:text></xsl:text>
                        </xsl:otherwise>
                ...
                <xsl:if test="count( $test_result_for_toolset ) > 0 and $log_file != ''">
                    <xsl:message>Writing log file document
                        <xsl:value-of select="$log_file" />
                    </xsl:message>
                    <exsl:document doctype-public="-//W3C//DTD XHTML 1.0 Strict//EN" encoding="utf-8"
                        href="{$log_file}" indent="yes"
                        method="html">
                        ...
                        <html>
                            ...
                            <frameset border="0" cols="190px,*" frameborder="0" framespacing="0">
                                <frame name="tocframe" scrolling="auto" src="../toc.html" />
                                <frame name="docframe" scrolling="auto" src="../../{$log_file}" />
                            </frameset>
                        </html>
                    </exsl:document>
                </xsl:if>
