...
<xsl:stylesheet xmlns:exsl="http://exslt.org/common"
    ...
    xmlns:xsl="http://www.w3.org/1999/XSL/Transform" exclude-result-prefixes="set str meta"
                            ...
                            <xsl:for-each select="$sorted_libraries">
                                ...
                                <xsl:variable name="library_header">
                                    <td class="library-name">
                                        <a class="library-link" href="{$library_page}.html"
                                            target="_top">
                                ...
                                <tr class="summary-row{$line_mod}">
                                    ...
                                    <xsl:for-each select="$sorted_toolsets/toolset">
                                        ...
                                        <xsl:variable name="current_cell"
                                            select="$current_row[ @toolset=$toolset ]" />
                                        <xsl:choose>
    ...
    <xsl:template name="insert_cell_developer">
        ...
        <xsl:variable name="class">
            <xsl:choose>
                ...
                <xsl:when
                    test="count( $current_cell[@result='fail' and  @status='unexpected' and @is-new='no'] )">
                    <xsl:text>summary-fail-unexpected</xsl:text>
                </xsl:when>
                <xsl:when
                    test="count( $current_cell[@result='fail' and  @status='unexpected' and @is-new='yes'] )">
                    <xsl:text>summary-fail-unexpected-new</xsl:text>
                </xsl:when>
                <xsl:when
                    test="count( $current_cell[@result='success' and  @status='unexpected'] )">
        ...
        <td class="{$class}">
            <xsl:choose>
                <xsl:when test="$class='summary-unusable'">
                    <a class="log-link" href="{$library_page}.html" target="_top">
