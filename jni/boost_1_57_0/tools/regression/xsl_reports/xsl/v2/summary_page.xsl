...
<xsl:stylesheet xmlns:exsl="http://exslt.org/common"
    ...
    xmlns:xsl="http://www.w3.org/1999/XSL/Transform" exclude-result-prefixes="exsl func set meta"
    extension-element-prefixes="func exsl" version="1.0">
    ...
    <!-- necessary indexes -->
    <xsl:key name="library_test_name_key" match="test-log"
    ...
    <xsl:variable name="test_case_logs"
        select="//test-log[ meta:is_test_log_a_test_case(.) and meta:show_library( @library, $release ) and meta:show_toolset( @toolset, $release )]" />
    ...
    <xsl:variable name="sorted_libraries"
        select="exsl:node-set( $sorted_libraries_output )/library/@library" />
    ...
    <xsl:template match="/">
        ...
        <xsl:variable name="summary_results"
            select="concat( 'summary', $release_postfix, '_.html' )" />

        <!-- Summary page -->
        <html>
            <head>
                <link href="../master.css" rel="stylesheet" title="master" type="text/css" />
                <title>Boost regression summary:
            ...
            <frameset border="0" cols="190px,*" frameborder="0" framespacing="0">
                ...
                <frame name="docframe" scrolling="auto" src="{$summary_results}" />
            </frameset>
        </html>

        <!-- Summary results -->
        <xsl:message>Writing document
            <xsl:value-of select="$summary_results" />
        </xsl:message>

        <exsl:document doctype-public="-//W3C//DTD XHTML 1.0 Strict//EN" encoding="utf-8"
            href="{$summary_results}" indent="yes" method="html">
            ...
            <html>
                ...
                <body>
                    ...
                    <h1 class="page-title">
                        ...
                        <a class="hover-link" href="summary{$release_postfix}.html" target="_top">
                            <xsl:value-of select="$source" />
                        </a>
                    </h1>
                    ...
                    <div class="statistics">
                        Unusable:
                        <xsl:value-of
                            select="count( $test_case_logs[ meta:test_case_status( $explicit_markup, . ) = 'unusable' ] )" />
                        &#160;|&#160; Regressions:
                        <xsl:value-of
                            select="count( $test_case_logs[ meta:test_case_status( $explicit_markup, . ) = 'fail-unexpected' ] )" />
                        &#160;|&#160; New failures:
                        <xsl:value-of
                            select="count( $test_case_logs[ meta:test_case_status( $explicit_markup, . ) = 'fail-unexpected-new' ] )" />
                    </div>

                    <!-- summary table -->

                    <table border="0" cellpadding="0" cellspacing="0" class="summary-table"
                        summary="Overall summary" width="1%">
                        ...
                        <tbody>
                            ...
                            <xsl:for-each select="$sorted_libraries">
                                ...
                                <xsl:variable name="library_page"
                                    select="meta:encode_path( $library )" />
                                <xsl:variable name="current_row"
                                    select="$test_logs[ @library=$library ]" />

                                <xsl:variable name="expected_test_count"
                                    select="count( $current_row[ generate-id(.) = generate-id( key('test_name_key',@test-name)[1] ) ] )" />
                                <xsl:variable name="library_header">
                                    <td class="library-name">
                                        <a class="library-link"
                                            href="{$library_page}{$release_postfix}.html" target="_top">
                                ...
                                <xsl:variable name="line_mod">
                                    <xsl:choose>
                                        ...
                                        <xsl:when
                                            test="generate-id( . ) = generate-id( $sorted_libraries[1] )">
                                            <xsl:text>-first</xsl:text>
                                        </xsl:when>
                                        <xsl:when
                                            test="generate-id( . ) = generate-id( $sorted_libraries[ last() ] )">
                                ...
                                <tr class="summary-row{$line_mod}">
                                    ...
                                    <xsl:for-each
                                        select="$run_toolsets/platforms/platform/runs/run/toolset">
                                        <xsl:variable name="toolset" select="@name" />
                                        <xsl:variable name="runner" select="../@runner" />

                                        <xsl:variable name="current_cell"
                                            select="$current_row[ @toolset=$toolset and ../@runner = $runner ]" />

                                        <xsl:choose>
                                            <xsl:when test="$mode='user'">
                                                <xsl:call-template name="insert_cell_user">
                                                    <xsl:with-param name="current_cell"
                                                        select="$current_cell" />
                                                    <xsl:with-param name="library"
                                                        select="$library" />
                                                    <xsl:with-param name="toolset"
                                                        select="$toolset" />
                                                    <xsl:with-param name="expected_test_count"
                                                        select="$expected_test_count" />
                                                </xsl:call-template>
                                            </xsl:when>
                    ...
                    <div id="legend">
                        <xsl:copy-of
                            select="document( concat( 'html/summary_', $mode, '_legend.html' ) )" />
                    </div>
    ...
    <xsl:template name="insert_cell_developer">
        ...
        <xsl:variable name="class"
            select="concat( 'summary-', meta:result_cell_class( $library, $toolset, $current_cell ) )" />
        ...
        <td class="{$class}" title="{$library}/{$toolset}">
            <xsl:choose>
                <xsl:when test="$class='summary-unusable'">
                    <xsl:text>&#160;&#160;</xsl:text>
                    <a class="log-link" href="{$library_page}{$release_postfix}.html" target="_top">
