...
<xsl:stylesheet xmlns:exsl="http://exslt.org/common"
    ...
    exclude-result-prefixes="exsl set meta" extension-element-prefixes="func exsl" version="1.0">
    ...
    <!-- necessary indexes -->
    <xsl:key name="test_name_key" match="test-log"
    ...
    <xsl:key name="library-name_toolset-name_key" match="unusables/unusable"
    ...
    <xsl:template name="test_type_col">
        <td class="test-type">
            <a class="legend-link"
                href="http://www.boost.org/status/compiler_status.html#Understanding" target="_top">
                <xsl:variable name="test_type" select="./@test-type" />
                <xsl:choose>
                    <xsl:when test="$test_type='run_pyd'">
                        <xsl:text>r</xsl:text>
                    </xsl:when>
                    <xsl:when test="$test_type='run_mpi'">
                        <xsl:text>r</xsl:text>
                    </xsl:when>
                    ...
                    <xsl:otherwise>
                        <xsl:message terminate="yes">Incorrect test type "<xsl:value-of
                            select="$test_type" />"
                        </xsl:message>
                    </xsl:otherwise>
                </xsl:choose>
    ...
    <xsl:template match="/">
        ...
        <xsl:message>
            <xsl:value-of select="count($unusables)" />
            <xsl:copy-of select="$unusables" />
        </xsl:message>

        <exsl:document encoding="utf-8" href="debug.xml" indent="yes" method="xml">
        ...
        <head>
            ...
            <title>Boost regression:
                <xsl:value-of select="$source" />
            </title>
        </head>
        <frameset border="0" cols="190px,*" frameborder="0" framespacing="0">
            <frame name="tocframe" scrolling="auto" src="toc{$release_postfix}.html" />
            <frame name="docframe" scrolling="auto" src="{$index_path}" />
        </frameset>

        <!-- Index content -->
        <xsl:message>Writing document
            <xsl:value-of select="$index_path" />
        </xsl:message>

        <exsl:document doctype-public="-//W3C//DTD XHTML 1.0 Strict//EN" encoding="utf-8"
            href="{$index_path}" indent="yes" method="html">
            ...
            <html>
                ...
                <body>
                    ...
                    <img align="right" alt="Boost logo" border="0" height="86"
                        src="http://www.boost.org/boost.png" width="277"></img>
                    ...
                    <xsl:variable name="purpose">
                        <xsl:choose>
                            <xsl:when test="$mode='user'">
                                The purpose of this report is to help a user to find out whether a
                                particular library
                                works on the particular compiler(s). For SVN "health report", see
                                <a href="../{$alternate_mode}/index.html" target="_top">developer
                                    summary</a>.
                            </xsl:when>
                            <xsl:when test="$mode='developer'">
                                Provides Boost developers with visual indication of the SVN
                                "health". For user-level
                                report, see <a href="../{$alternate_mode}/index.html" target="_top">
                                user summary</a>.
                            </xsl:when>
                        </xsl:choose>
                    </xsl:variable>
        ...
        <xsl:if test="$multiple.libraries">
            ...
            <xsl:message>Writing document
                <xsl:value-of select="$toc_path" />
            </xsl:message>

            <exsl:document doctype-public="-//W3C//DTD XHTML 1.0 Strict//EN" encoding="utf-8"
                href="{$toc_path}" indent="yes" method="html">

                <html>
                    <head>
                        <link href="../master.css" rel="stylesheet" title="master"
                            type="text/css" />
                    </head>
                    <body class="{$mode}-toc">
                        <div class="toc-header-entry">
                            <a class="toc-entry" href="index{$release_postfix}.html" target="_top">
                                Report info
                            </a>
                        </div>
                        <div class="toc-header-entry">
                            <a class="toc-entry" href="summary{$release_postfix}.html"
                                target="_top">Summary
                            </a>
                        </div>
                        ...
                        <xsl:if test="$mode='developer'">
                            <div class="toc-header-entry">
                                <a class="toc-entry" href="issues.html" target="_top">Unresolved
                                    issues
                                </a>
                            </div>
                        </xsl:if>
                        ...
                        <xsl:for-each select="$libraries">
                            ...
                            <div class="toc-entry">
                                <a class="toc-entry" href="{$library_page}{$release_postfix}.html"
                                    target="_top">
                                    <xsl:value-of select="." />
                                </a>
                            </div>
        ...
        <xsl:for-each select="$libraries[ meta:show_library( ., $release )]">
            ...
            <xsl:variable name="library_results"
                select="concat( meta:encode_path( $library ), $release_postfix, '_.html' )" />
            <xsl:variable name="library_page"
                select="concat( meta:encode_path( $library ), $release_postfix, '.html' )" />

            <!-- Library page -->
            <xsl:message>Writing document
                <xsl:value-of select="$library_page" />
            </xsl:message>

            <exsl:document doctype-public="-//W3C//DTD XHTML 1.0 Strict//EN" encoding="utf-8"
                href="{$library_page}" indent="yes" method="html">

                <html>
                    <head>
                        <link href="../master.css" rel="stylesheet" title="master"
                            type="text/css" />
                        <title>Boost regression:<xsl:value-of select="$library" />/
                            <xsl:value-of select="$source" />
                        </title>
                    </head>
                    <frameset border="0" cols="190px,*" frameborder="0" framespacing="0">
                        <frame name="tocframe" scrolling="auto" src="toc{$release_postfix}.html" />
                        <frame name="docframe" scrolling="auto" src="{$library_results}" />
                    </frameset>
                </html>
            </exsl:document>
            ...
            <xsl:message>Writing document
                <xsl:value-of select="$library_results" />
            </xsl:message>

            <exsl:document doctype-public="-//W3C//DTD XHTML 1.0 Strict//EN" encoding="utf-8"
                href="{$library_results}" indent="yes" method="html">
                ...
                <html>
                    ...
                    <body>
                        ...
                        <h1 class="page-title">
                            <a name="{$library}" class="hover-link"
                                href="http://www.boost.org/libs/{$library}" target="_top">
                        ...
                        <xsl:variable name="library_marks"
                            select="$explicit_markup//library[ @name = $library ]/mark-unusable/toolset[  meta:re_match( @name, $run_toolsets//toolset/@name ) ]/.." />

                        <table border="0" cellpadding="0" cellspacing="0" class="library-table"
                            summary="Library results" width="1%">
                            ...
                            <tbody>
                                <xsl:variable name="lib_tests"
                                    select="$test_case_logs[@library = $library]" />
                                ...
                                <xsl:call-template name="insert_test_section">
                                    <xsl:with-param name="library" select="$library" />
                                    <xsl:with-param name="section_test_names"
                                        select="$lib_tests_by_category" />
                        ...
                        <xsl:if test="count( $library_marks/note ) > 0 ">
                            <table border="0" cellpadding="0" cellspacing="0"
                                class="library-library-notes" summary="library notes">
                        ...
                        <div id="legend">
                            <xsl:copy-of
                                select="document( concat( 'html/library_', $mode, '_legend.html' ) )" />
                        </div>
    ...
    <xsl:template name="insert_test_result">
        ...
        <xsl:choose>
            <xsl:when test="$log_link != ''">
                <xsl:text>&#160;&#160;</xsl:text>
                <a class="log-link" href="{$log_link}" target="_top">
    ...
    <xsl:template name="insert_cell_developer">
        ...
        <xsl:variable name="class"
            select="concat( 'library-', meta:result_cell_class( $library, $toolset, $test_log ) )" />

        <xsl:variable name="cell_link">
            <xsl:choose>
                <xsl:when test="count( $test_log ) &gt; 1">
                    <xsl:variable name="variants__file_path"
                        select="concat( meta:encode_path( concat( $test_log/../@runner, '-', $test_log/@library, '-', $test_log/@toolset, '-', $test_log/@test-name, '-variants_', $release_postfix ) ), '.html' )" />
                    <xsl:value-of select="$variants__file_path" />
                </xsl:when>
                <xsl:otherwise>
                    <xsl:value-of
                        select="meta:log_file_path( $test_log, $test_log/../@runner, $release_postfix )" />
                </xsl:otherwise>
            </xsl:choose>
        </xsl:variable>
        ...
        <td class="{$class}" title="{$test_log/@test-name}/{$toolset}">
            <xsl:choose>
                ...
                <xsl:when
                    test="count( $test_log[ @result != 'success' and @status = 'expected' ] ) &gt; 0">
                    <xsl:call-template name="insert_test_result">
                ...
                <xsl:when
                    test="$test_log/@result != 'success' and $test_log/@status = 'unexpected'">
                    <xsl:call-template name="insert_test_result">
    ...
    <xsl:template name="insert_test_line">
        ...
        <xsl:variable name="test_header">
            <td class="test-name">
                <a class="test-link" href="http://svn.boost.org/svn/boost/{$source}/{$test_program}"
                    target="_top">
        ...
        <tr class="library-row{$line_mod}">
            ...
            <xsl:for-each select="$run_toolsets/platforms/platform/runs/run/toolset">
                ...
                <xsl:variable name="test_result_for_toolset"
                    select="$test_results[ @toolset = $toolset and ../@runner=$runner ]" />
    ...
    <xsl:template name="insert_test_section">
        ...
        <xsl:variable name="category_span"
            select="count($toolsets/platforms/platform/runs/run/toolset) + 3" />

        <xsl:for-each select="$section_test_names">

            <xsl:variable name="test_name" select="@test-name" />
            <xsl:variable name="category_start"
                select="position() = 1 or @category != preceding-sibling::*[1]/@category" />
            <xsl:variable name="category_end"
                select="position() = last() or @category != following-sibling::*[1]/@category" />

            <xsl:variable name="line_mod">
                <xsl:choose>
                    <xsl:when test="$category_start and $category_end">
                        <xsl:text>-single</xsl:text>
                    </xsl:when>
                    <xsl:when test="$category_start">
                        <xsl:text>-first</xsl:text>
                    </xsl:when>
                    <xsl:when test="$category_end">
            ...
            <xsl:if test="$category_start and @category != '0'">
                <tr>
                    <td align="center" class="library-test-category-header"
                        colspan="{$category_span}">
            ...
            <xsl:call-template name="insert_test_line">
                <xsl:with-param name="library" select="$library" />
                <xsl:with-param name="test_results"
                    select="$lib_tests[ @test-name = $test_name ]" />
    ...
    <func:function name="meta:order_tests_by_category">
        ...
        <xsl:variable name="a">
            <xsl:for-each select="$tests">
                <xsl:sort order="ascending" select="concat( @category, '|', @test-name )" />
                <xsl:copy-of select="." />
            </xsl:for-each>
        </xsl:variable>
