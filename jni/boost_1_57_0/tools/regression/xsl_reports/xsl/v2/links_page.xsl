...
<xsl:stylesheet xmlns:exsl="http://exslt.org/common"
    ...
    xmlns:xsl="http://www.w3.org/1999/XSL/Transform" exclude-result-prefixes="meta"
    extension-element-prefixes="func exsl str set" version="1.0">
    ...
    <xsl:key name="library-name_toolset-name_key" match="unusable"
    ...
    <xsl:template match="/">
        ...
        <xsl:variable name="libs_test_test_log_tree"
            select="meta:restructure_logs( $test_logs_to_show )" />
        ...
        <xsl:for-each select="$libs_test_test_log_tree//toolset">
            ...
            <xsl:message>Processing test "<xsl:value-of select="$runner_id" />/<xsl:value-of
                select="$library_name" />/<xsl:value-of select="$test_name" />/<xsl:value-of
                select="$toolset_name" />"
            </xsl:message>
            ...
            <xsl:if test="count( $toolset/* ) &gt; 1">
                ...
                <xsl:variable name="variants_file_path"
                    select="meta:output_file_path( concat( $runner_id, '-', $library_name, '-', $toolset_name, '-', $test_name, '-variants' ) )" />
                ...
                <xsl:for-each select="str:tokenize( string( ' |_release' ), '|')">
                    ...
                    <xsl:for-each select="str:tokenize( string( 'developer|user' ), '|')">
                        ...
                        <xsl:variable name="variants__file_path"
                            select="concat( $directory, '/', meta:encode_path( concat( $runner_id, '-', $library_name, '-', $toolset_name, '-', $test_name, '-variants_', $release_postfix ) ), '.html' )" />

                        <xsl:call-template name="write_variants_reference_file">
                            <xsl:with-param name="path" select="$variants__file_path" />
                            <xsl:with-param name="variants_file_path"
                                select="concat( '../', $variants_file_path )" />
            ...
            <xsl:for-each select="./test-log">
                ...
                <xsl:if test="meta:show_output( $explicit_markup, $test_log )">
                    <xsl:variable name="log_file_path"
                        select="meta:log_file_path( ., $runner_id )" />
                    ...
                    <xsl:for-each select="str:tokenize( string( ' |_release' ), '|')">
                        ...
                        <xsl:for-each select="str:tokenize( string( 'developer|user' ), '|')">
                            ...
                            <xsl:variable name="reference_file_path"
                                select="concat( $directory, '/', meta:log_file_path( $test_log, $runner_id, $release_postfix ) )" />
    ...
    <func:function name="meta:restructure_logs">
        ...
        <xsl:variable name="fragment">
            <runner revision="{$test_logs[1]/../@revision}" runner_id="{$test_logs[1]/../@runner}"
                timestamp="{$test_logs[1]/../@timestamp}">
                <xsl:for-each select="$libs">
                    <xsl:variable name="library_name" select="." />
                    <xsl:variable name="library_test_logs"
                        select="$test_logs[@library=$library_name]" />
                    <library name="{$library_name}">
                        <xsl:variable name="tests"
                            select="set:distinct( $library_test_logs/@test-name )" />
                        <xsl:for-each select="$tests">
                            <xsl:variable name="test_name" select="." />
                            <xsl:variable name="test_test_logs"
                                select="$library_test_logs[@test-name=$test_name]" />
                            <test name="{$test_name}">
                                <xsl:variable name="toolsets"
                                    select="set:distinct( $test_test_logs/@toolset )" />
                                <xsl:for-each select="$toolsets">
                                    <xsl:variable name="toolset" select="." />
                                    <xsl:variable name="toolset_test_logs"
                                        select="$test_test_logs[@toolset=$toolset]" />
    ...
    <xsl:template name="write_variants_reference_file">
        ...
        <xsl:message>Writing variants reference file
            <xsl:value-of select="$path" />
        </xsl:message>
        <exsl:document doctype-public="-//W3C//DTD XHTML 1.0 Strict//EN" encoding="utf-8"
            href="{$path}" indent="yes" method="html">
            <html>
                <head>
                ...
                <frameset border="0" cols="190px,*" frameborder="0" framespacing="0">
                    ...
                    <frame name="docframe" scrolling="auto" src="{$variants_file_path}" />
                </frameset>
            </html>
        </exsl:document>
    ...
    <func:function name="meta:output_page_header">
        ...
        <xsl:choose>
            <xsl:when test="$test_log/@test-name != ''">
                <func:result
                    select="concat( $runner_id, ' - ', $test_log/@library, ' - ', $test_log/@test-name, ' / ', $test_log/@toolset )" />
            </xsl:when>
            <xsl:otherwise>
    ...
    <xsl:template name="write_variants_file">
        ...
        <xsl:message>Writing variants file
            <xsl:value-of select="$path" />
        </xsl:message>
        <exsl:document doctype-public="-//W3C//DTD XHTML 1.0 Strict//EN" encoding="utf-8"
            href="{$path}" indent="yes" method="html">
            ...
            <html>
                <xsl:variable name="component"
                    select="meta:output_page_header( $test_logs[1], $runner_id )" />
    ...
    <xsl:template name="write_test_result_file">
        ...
        <exsl:document doctype-public="-//W3C//DTD XHTML 1.0 Strict//EN" encoding="utf-8"
            ...
            <html>
                <xsl:variable name="component"
                    select="meta:output_page_header( $test_log, $runner_id )" />
                <xsl:variable name="age"
                    select="meta:timestamp_difference( $timestamp, $run_date )" />
                ...
                <body>
                    ...
                    <xsl:if test="compile">
                        <p>
                            <div class="log-compiler-output-title">Compile [<xsl:value-of
                                select="compile/@timestamp" />]:
                                <span class="output-{compile/@result}">
                                    <xsl:value-of select="compile/@result" />
                                </span>
                            </div>
                            <pre>
                    ...
                    <xsl:if test="link">
                        <p>
                            <div class="log-linker-output-title">Link [<xsl:value-of
                                select="link/@timestamp" />]:
                                <span class="output-{link/@result}">
                                    <xsl:value-of select="link/@result" />
                                </span>
                            </div>
                            <pre>
                    ...
                    <xsl:if test="lib">
                        <p>
                            <div class="log-linker-output-title">Lib [<xsl:value-of
                                select="lib/@timestamp" />]:
                                <span class="output-{lib/@result}">
                                    <xsl:value-of select="lib/@result" />
                                </span>
                            </div>
                            <p>
                                See
                                <a href="{meta:encode_path( concat( $runner_id, '-',  lib/node() )  ) }.html">
                    ...
                    <xsl:if test="run">
                        <p>
                            <div class="log-run-output-title">Run [<xsl:value-of
                                select="run/@timestamp" />]:
                                <span class="output-{run/@result}">
                                    <xsl:value-of select="run/@result" />
                                </span>
                            </div>
                            <pre>
    ...
    <xsl:template name="write_test_results_reference_file">
        ...
        <xsl:message>Writing log frame document
            <xsl:value-of select="$path" />
        </xsl:message>
        <exsl:document doctype-public="-//W3C//DTD XHTML 1.0 Strict//EN" encoding="utf-8"
            href="{$path}" indent="yes" method="html">
            ...
            <html>
                ...
                <frameset border="0" cols="190px,*" frameborder="0" framespacing="0">
                    ...
                    <frame name="docframe" scrolling="auto" src="../../{$log_file_path}" />
                </frameset>
            </html>
        </exsl:document>
    </xsl:template>
