...
<xsl:stylesheet xmlns:exsl="http://exslt.org/common"
    ...
    xmlns:xsl="http://www.w3.org/1999/XSL/Transform" exclude-result-prefixes="exsl set meta"
    ...
    <xsl:variable name="failing_tests" select="//test-log[@status='unexpected' and @result='fail'
    ...
    <xsl:template match="/">
        <xsl:variable name="issues_list" select="concat('issues', $release_postfix, '_.html')" />
                ...
                <body>
                    ...
                    <xsl:call-template name="insert_report_header">
                        ...
                        <xsl:with-param name="purpose"
                            select="'Provides a list of current unresolved test failures.'" />
                    </xsl:call-template>
                    ...
                    <xsl:for-each select="$libraries">
                        ...
                        <xsl:variable name="library_tests"
                            select="$failing_tests[@library = $library]" />
                        <xsl:variable name="library_test_names"
                            select="set:distinct( $library_tests/@test-name )" />

                        <h2>
                            <a name="{$library}" />
                            <a class="hover-link" href="{$library_page}{$release_postfix}.html"
                                target="_top">
                                <xsl:value-of select="$library" />
                                <xsl:text>(</xsl:text>
                            ...
                            <tbody>
                                <xsl:for-each select="$library_test_names">
                                    ...
                                    <xsl:variable name="unexpected_toolsets"
                                        select="$library_tests[@test-name = $test_name]/@toolset" />
                                    ...
                                    <tr>
                                        <td class="test-name">
                                            <a class="test-link"
                                                href="http://svn.boost.org/svn/boost/{$source}/{$test_program}" target="_top">
                                            ...
                                            <table class="issue-box"
                                                ...
                                                <tr class="library-row-single">
                                                    <xsl:for-each select="$unexpected_toolsets">
                                                        ...
                                                        <xsl:variable name="test_logs" select="$library_tests[@test-name = $test_name
                                                     and @toolset = $toolset]" />
                                                        <xsl:for-each select="$test_logs">
                                                            <xsl:call-template
                                                                name="print_failure_cell">
                                                                <xsl:with-param name="test_log"
                                                                    select="." />
        ...
        <exsl:document encoding="utf-8" href="issues-email.txt" method="text">
