...
<xsl:stylesheet xmlns:date="http://exslt.org/dates-and-times"
    ...
    xmlns:meta="http://www.meta-comm.com" xmlns:set="http://exslt.org/sets"
    xmlns:str="http://exslt.org/strings" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    exclude-result-prefixes="exsl func date str set meta" extension-element-prefixes="func"
    version="1.0">
                    ...
                    <platform name="{$platform}">
                        <runs>
                            <xsl:for-each select="$runs[ @platform = $platform ]">
                                ...
                                <run platform="{@platform}" revision="{@revision}"
                                    run-type="{@run-type}" runner="{@runner}" source="{@source}"
                                    timestamp="{@timestamp}">
                                    ...
                                    <xsl:variable name="not_ordered_toolsets"
                                        select="set:distinct( .//test-log[ meta:is_test_log_a_test_case(.) and meta:show_toolset( @toolset, $release ) ]/@toolset ) " />
                                    ...
                                    <xsl:variable name="not_ordered_toolsets_with_info_f">
                                        <xsl:for-each select="$not_ordered_toolsets">
                                            ...
                                            <xsl:variable name="required">
                                                <xsl:choose>
                                                    <xsl:when
                                                        test="count( $required_toolsets[ @name = $toolset ] ) > 0">
                                                        yes
                                                    </xsl:when>
                                            ...
                                            <xsl:variable name="required_sort_hint">
                                                <xsl:choose>
                                                    <xsl:when test="$required = 'yes'">sort hint A
                                                    </xsl:when>
                                                    <xsl:otherwise>sort hint B</xsl:otherwise>
                                                </xsl:choose>
                                            </xsl:variable>
                                            <toolset name="{$toolset}" required="{$required}"
                                                required_sort_hint="{$required_sort_hint}" />
                                        </xsl:for-each>
                                    </xsl:variable>

                                    <xsl:variable name="not_ordered_toolsets_with_info"
                                        select="exsl:node-set( $not_ordered_toolsets_with_info_f )" />

                                    <xsl:for-each select="$not_ordered_toolsets_with_info/toolset">
                                        <xsl:sort order="ascending"
                                            select="concat( @required_sort_hint, '-', @name )" />
                                        <xsl:copy-of select="." />
                                    </xsl:for-each>
                                </run>
                            </xsl:for-each>
    ...
    <func:function name="meta:test_case_status">
        ...
        <xsl:variable name="status">
            <xsl:choose>
                ...
                <xsl:when
                    test="$test_log/@result='fail' and  $test_log/@status='unexpected' and $test_log/@is-new='no'">
                    <xsl:text>fail-unexpected</xsl:text>
                </xsl:when>
                <xsl:when
                    test="$test_log/@result='fail' and  $test_log/@status='unexpected' and $test_log/@is-new='yes'">
    ...
    <func:function name="meta:is_toolset_required">
        ...
        <func:result
            select="count( $explicit_markup/explicit-failures-markup/mark-toolset[ @name = $toolset and @status='required' ] ) > 0" />
    </func:function>

    <func:function name="meta:is_library_beta">
        <xsl:param name="library" />
        <func:result
            select="count( $explicit_markup/explicit-failures-markup/library[ @name = $library and @status='beta' ] ) > 0" />
    </func:function>
    ...
    <func:function name="meta:is_unusable_">
        ...
        <func:result
            select="count( $explicit_markup//library[ @name = $library ]/mark-unusable/toolset[ meta:re_match( @name, $toolset ) ] ) > 0" />
    </func:function>
    ...
    <func:function name="meta:re_match">
        ...
        <xsl:choose>
            ...
            <xsl:when
                test="substring( $pattern, 1, 1 ) = '*' and substring( $pattern, string-length($pattern), 1 ) = '*' ">
                <func:result
                    select="contains( $text, substring( $pattern, 2, string-length($pattern) - 2 ) ) " />
            </xsl:when>
            <xsl:when test="substring( $pattern, 1, 1 ) = '*'">
                <xsl:variable name="pattern_tail"
                    select="substring( $pattern, 2, string-length($pattern) - 1 )" />
                <func:result
                    select="substring( $text, string-length($text) - string-length($pattern_tail) + 1, string-length($pattern_tail) ) = $pattern_tail" />
            </xsl:when>
            <xsl:when test="substring( $pattern, string-length($pattern), 1 ) = '*' ">
                <xsl:variable name="pattern_head"
                    select="substring( $pattern, 1, string-length($pattern) - 1 )" />
            ...
            <xsl:when test="contains( $pattern, '*' ) ">
                ...
                <func:result
                    select="starts-with( $text, $pattern_head ) and substring( $text, string-length($text) - string-length($pattern_tail) + 1, string-length($pattern_tail) ) = $pattern_tail" />
            </xsl:when>
        </xsl:choose>
    </func:function>
        ...
        <xsl:choose>
            ...
            <xsl:otherwise>
                <xsl:variable name="time"
                    select="substring-before( date:time( $timestamp ), 'Z' )" />
                ...
                <func:result
                    select="concat( $day_abbrev, ', ', $day, ' ', $month_abbrev, ' ', $year, ' ', $time, ' +0000' )" />
            </xsl:otherwise>
        </xsl:choose>
    ...
    <func:function name="meta:output_file_path">
        <xsl:param name="path" />
        <func:result
            select="concat( $output_directory, '/', meta:encode_path( $path ), '.html' )" />
    </func:function>
        ...
        <func:result>
            <xsl:choose>
                <xsl:when test="meta:show_output( $explicit_markup, $test_log )">
                    <xsl:value-of
                        select="meta:output_file_path( concat( $runner, '-', $test_log/@target-directory, $release_postfix ) )" />
                </xsl:when>
                <xsl:otherwise>
                    <xsl:text></xsl:text>
                </xsl:otherwise>
    ...
    <func:function name="meta:result_cell_class">
        ...
        <func:result>
            <xsl:choose>
                ...
                <xsl:when
                    test="count( $test_logs[@result='fail' and  @status='unexpected' and @is-new='no'] )">
                    <xsl:text>fail-unexpected</xsl:text>
                </xsl:when>

                <xsl:when
                    test="count( $test_logs[@result='fail' and  @status='unexpected' and @is-new='yes'] )">
                    <xsl:text>fail-unexpected-new</xsl:text>
                </xsl:when>
    ...
    <xsl:template name="insert_report_header">
        ...
        <div class="report-info">
            <div>
                <b>Report Time:</b>
                <xsl:value-of select="meta:format_timestamp( $run_date )" />
            </div>
                ...
                <xsl:for-each select="str:split( $warnings, '+' )">
                    <div class="report-warning">
                        ...
                        <a class="warning-link"
                            href="mailto:boost-testing@lists.boost.org?subject=[Report Pages] {.} ({meta:format_timestamp( $run_date )})">
                            <xsl:value-of select="." />
                        </a>
                    </div>
                </xsl:for-each>
    ...
    <xsl:template name="insert_view_link">
        ...
        <xsl:choose>
            <xsl:when test="$release='yes'">
                <a class="{$class}" href="{$page}.html" target="_top">
                    <xsl:text>Full View</xsl:text>
                </a>
            </xsl:when>
            <xsl:otherwise>
                <a class="{$class}" href="{$page}_release.html" target="_top">
    ...
    <xsl:template name="insert_page_links">
        ...
        <div class="links">
            ...
            <xsl:text>&#160;|&#160;</xsl:text>
            <a class="view-link" href="../{$mode}/{$page}{$release_postfix}.html" target="_top">
    ...
    <xsl:template name="insert_runners_rows">
            ...
            <tr>
                ...
                <xsl:for-each select="$run_toolsets/platforms/platform">
                    <xsl:if test="count(./runs/run/toolset) &gt; 0">
                        <td class="runner" colspan="{count(./runs/run/toolset)}">
        ...
        <tr>
            ...
            <xsl:for-each select="$run_toolsets//runs/run[ count(toolset) > 0 ]">
                <td class="runner" colspan="{count(toolset)}">
                    <a href="../{@runner}.html">
        ...
        <tr>
            <td colspan="{$colspan}">&#160;</td>
            <xsl:for-each select="$run_toolsets//runs/run[ count(toolset) > 0 ]">
                <td class="revision" colspan="{count(toolset)}">
                    rev
        ...
        <tr>
            <td colspan="{$colspan}">&#160;</td>
            <xsl:for-each select="$run_toolsets//runs/run[ count(toolset) > 0 ]">
                <xsl:variable name="timestamp_diff"
                    select="meta:timestamp_difference( @timestamp, $run_date )" />
                <xsl:variable name="age"
                    select="meta:iif( $timestamp_diff &lt; 30, $timestamp_diff, 30 )" />
                <td class="timestamp" colspan="{count(toolset)}">
                    <span class="timestamp-{$age}">
                        <xsl:value-of select="meta:format_timestamp( @timestamp )" />
                    </span>
                    <xsl:if test="@run-type != 'full'">
                        <span class="run-type-{@run-type}">
                            <xsl:value-of select="substring( @run-type, 1, 1 )" />
                        </span>
                    </xsl:if>
                </td>
            </xsl:for-each>
    ...
    <xsl:template name="insert_toolsets_row">
        ...
        <tr valign="middle">
            ...
            <xsl:variable name="title">
                <xsl:choose>
                    <xsl:when test="$mode = 'summary'">&#160;library&#160;/&#160;toolset&#160;
                    </xsl:when>
                    <xsl:when test="$mode = 'details'">&#160;test&#160;/&#160;toolset&#160;
                    </xsl:when>
                </xsl:choose>
            </xsl:variable>

            <td class="head" colspan="{$colspan}" width="1%">
                        ...
                        <xsl:if test="$mode = 'details'">
                            ...
                            <xsl:variable name="toolset_notes_fragment">
                                <xsl:for-each select="$library_marks/note">
                                    <xsl:if
                                        test="count( ../toolset[meta:re_match( @name, $toolset )] ) > 0">
                                        <note index="{position()}" />
                                    </xsl:if>
                                </xsl:for-each>
                            </xsl:variable>
                            ...
                            <xsl:if test="count( $toolset_notes ) > 0">
                                <span class="super">
                                    <xsl:for-each select="$toolset_notes">
                                        ...
                                        <xsl:if
                                            test="generate-id( . ) != generate-id( $toolset_notes[1] )">
                                            ,
                                        </xsl:if>
                                        <a href="#{$library}-note-{$note_index}"
        ...
        <div class="note">
            ...
            <xsl:if test="$references">
                ...
                <xsl:for-each select="str:tokenize($references, ',')">
                    ...
                    <xsl:variable name="referenced_note"
                        select="$explicit_markup//note[ $ref_id = @id ]" />
