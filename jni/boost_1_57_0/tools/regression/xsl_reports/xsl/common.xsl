...
<xsl:stylesheet xmlns:exsl="http://exslt.org/common"
    ...
    xmlns:xsl="http://www.w3.org/1999/XSL/Transform" extension-element-prefixes="func" version="1.0">
    ...
    <xsl:template name="get_toolsets">
        ...
        <xsl:for-each select="exsl:node-set( $toolset_output )/toolset">
            <xsl:sort order="ascending" select="concat( @sort, ' ', @toolset)" />
            <xsl:copy-of select="." />
        </xsl:for-each>
    ...
    <func:function name="meta:show_output">
        ...
        <func:result
            select="$test_log/@result != 'success' and not( meta:is_unusable( $explicit_markup, $test_log/@library, $test_log/@toolset )) or $test_log/@show-run-output = 'true'" />
    </func:function>

    <func:function name="meta:is_test_log_a_test_case">
        <xsl:param name="test_log" />
        <func:result
            select="$test_log/@test-type='compile' or $test_log/@test-type='compile_fail' or $test_log/@test-type='run' or $test_log/@test-type='run_pyd'" />
    </func:function>
    ...
    <func:function name="meta:is_unusable">
        ...
        <func:result
            select="$explicit_markup//library[ @name = $library ]/mark-unusable[ toolset/@name = $toolset or toolset/@name='*' ]" />
    </func:function>
        ...
        <xsl:choose>
            ...
            <xsl:when test="substring( $pattern, string-length($pattern), 1 ) = '*' ">
                <xsl:variable name="pattern_head"
                    select="substring( $pattern, 1, string-length($pattern) - 2 )" />
                <func:result
                    select="substring( $text, 1, string-length($pattern_head) ) = $pattern_head " />
            </xsl:when>
        </xsl:choose>
    </func:function>
