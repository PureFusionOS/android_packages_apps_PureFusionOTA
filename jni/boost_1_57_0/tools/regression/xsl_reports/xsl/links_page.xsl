...
<xsl:stylesheet xmlns:exsl="http://exslt.org/common"
    ...
    <xsl:template match="test-log[ meta:show_output( $explicit_markup, . ) ]">
        ...
        <xsl:message>Writing log file document
            <xsl:value-of select="$document_path" />
        </xsl:message>

        <exsl:document doctype-public="-//W3C//DTD XHTML 1.0 Strict//EN" encoding="utf-8"
            href="{$document_path}" indent="yes" method="html">
            ...
            <html>
                    ...
                    <xsl:choose>
                        <xsl:when test="@test-name != ''">
                            <div class="log-test-title">
                                <xsl:value-of
                                    select="concat( @library, ' - ', @test-name, ' / ', @toolset )" />
                            </div>
                        </xsl:when>
                        <xsl:otherwise>
                ...
                <head>
                    ...
                    <title>Boost regression - test run output:
                    ...
                    <div>
                        ...
                        <xsl:if test="notes/note">
                            <p>
                                ...
                                <xsl:call-template name="show_notes">
                                    ...
                                    <xsl:with-param name="explicit_markup"
                                        select="$explicit_markup" />
                                </xsl:call-template>
                            </p>
                        </xsl:if>
                        ...
                        <xsl:if test="lib">
                            <p>
                                ...
                                <p>
                                    See
                                    <a href="{meta:encode_path( lib/node() )}.html">
