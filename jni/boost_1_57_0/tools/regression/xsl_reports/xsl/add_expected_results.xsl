    ...
    <xsl:template match="test-log">
        ...
        <xsl:element name="{local-name()}">
            ...
            <xsl:variable name="expected_results_test_case"
                select="$expected_results//*/test-result[ @library=$library and ( @test-name=$test-name or @test-name='*' ) and @toolset = $toolset]" />
            <xsl:variable name="new_failures_markup"
                select="$failures_markup//library[@name=$library]/mark-expected-failures[ meta:re_match( test/@name, $test-name ) and meta:re_match( toolset/@name, $toolset ) ]" />
            <xsl:variable name="failures_markup"
                select="$failures_markup//library[@name=$library]/test[ meta:re_match( @name, $test-name ) ]/mark-failure[ meta:re_match( toolset/@name, $toolset ) ]" />
            ...
            <xsl:variable name="expected_result">
                <xsl:choose>
                    <xsl:when
                        test='count( $failures_markup ) &gt; 0 or count( $new_failures_markup ) &gt; 0'>
                        <xsl:text>fail</xsl:text>
                    </xsl:when>
