...
def list_dir(dir):
    ...
    for file_path in glob.glob(os.path.join(dir, "*.zip")):
        if os.path.isfile(file_path):
            ...
            mod_time = (
            mod_time[0], mod_time[1], mod_time[2], mod_time[3], mod_time[4], mod_time[5], 0, 0,
            mod_time[8])
            # no size (for now)
...
def diff(source_dir_content, destination_dir_content):
    ...
    for source_file in source_dir_content:
        ...
        elif time.mktime(found.date) != time.mktime(source_file.date):
            result[0].append(source_file.name)
        else:
            pass
...
class merge_xml_action(action):
    ...
    def update(self):
                ...
                def characters(self, content):
                    if not self.trimmed:
                        ...
                        if len(self.character_content) > max_size:
                            self.character_content = self.character_content[
        ...
        try:
            trimmed_source = filter_xml(self.source_,
                                        '%s-trimmed.xml' % os.path.splitext(self.source_)[0])
            utils.libxslt(
                utils.log
                , trimmed_source
...
class make_links_action(action):
    def __init__(self, source, destination, output_dir, tag, run_date, comment_file,
...
class unzip_action(action):
    ...
    def update(self):
        try:
            utils.log('  Unzipping "%s" ... into "%s"' % (
            shorten(self.source_), os.path.dirname(self.file_path_)))
...
def ftp_task(site, site_path, destination):
    ...
    source_content = [x for x in source_content if re.match(r'.+[.](?<!log[.])zip',
                                                            x.name) and x.name.lower() != 'boostbook.zip']
...
def unzip_archives_task(source_dir, processed_dir, unzip_func):
    utils.log('')
    utils.log('unzip_archives_task: unpacking updated archives in "%s" into "%s"...' % (
    source_dir, processed_dir))
    __log__ = 1

    target_files = [os.path.join(processed_dir, os.path.basename(x.replace(".zip", ".xml"))) for x
                    in glob.glob(os.path.join(source_dir, "*.zip"))] + glob.glob(
        os.path.join(processed_dir, "*.xml"))
    actions = [
        unzip_action(os.path.join(source_dir, os.path.basename(x.replace(".xml", ".zip"))), x,
                     unzip_func) for x in target_files]
    for a in actions:
        a.run()


def merge_xmls_task(source_dir, processed_dir, merged_dir, expected_results_file,
                    failures_markup_file, tag):
    utils.log('')
    utils.log('merge_xmls_task: merging updated XMLs in "%s"...' % source_dir)
    __log__ = 1

    utils.makedirs(merged_dir)
    target_files = [os.path.join(merged_dir, os.path.basename(x)) for x in
                    glob.glob(os.path.join(processed_dir, "*.xml"))] + glob.glob(
        os.path.join(merged_dir, "*.xml"))
...
def make_links_task(input_dir, output_dir, tag, run_date, comment_file, extended_test_results,
                    failures_markup_file):
    utils.log('')
    utils.log('make_links_task: make output files for test results in "%s"...' % input_dir)
    __log__ = 1

    target_files = [x + ".links" for x in glob.glob(os.path.join(input_dir, "*.xml"))] + glob.glob(
        os.path.join(input_dir, "*.links"))
...
def merge_processed_test_runs(test_runs_dir, tag, writer):
    utils.log('')
    utils.log(
        'merge_processed_test_runs: merging processed test runs from %s into a single XML...' % test_runs_dir)
    __log__ = 1
...
def execute_tasks(
    ...
    merge_xmls_task(incoming_dir, processed_dir, merged_dir, expected_results_file,
                    failures_markup_file, tag)
