...
def make_test_log(xml_generator
    ...
    if test_type != "lib":
        ...
        xml_generator.characters(
            "Compiling in %s" % make_library_target_directory(library_idx, toolset_idx))
        xml_generator.endElement("compile")
...
def make_expicit_failure_markup(num_of_libs, num_of_toolsets, num_of_tests):
    ...
    for i_toolset in range(0, num_of_toolsets):
        if i_toolset < 2:
            g.startElement("mark-toolset",
                           {"name": "toolset_%02d" % i_toolset, "status": "required"})
