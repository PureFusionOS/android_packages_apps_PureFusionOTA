            ...
            <xsl:choose>
                ...
                <xsl:when test="contains($inp, '&#x000A;')">
                    ...
                    <xsl:variable name="n_max">
                        <xsl:choose>
                            <xsl:when
                                test="string-length($str) > $char-per-cell and string-length($str) > $max">
                    ...
                    <xsl:choose>
                        ...
                        <xsl:otherwise>
                            <xsl:choose>
                                <xsl:when
                                    test="$n_max > $char-per-cell and $n_max > string-length($next)">
                                    <!-- set size with next.len -->
                                    <xsl:value-of select="round($char-per-cell div $n_max * 100)" />
                                </xsl:when>
                                <xsl:when
                                    test="string-length($next) > $char-per-cell and string-length($next) > $n_max">
                                    <!-- set size with n_max -->
                                    <xsl:value-of
                                        select="round($char-per-cell div string-length($next) * 100)" />
                                </xsl:when>
                ...
                <xsl:otherwise>
                    <xsl:choose>
                        <xsl:when test="string-length($inp) > $char-per-cell">
                            ...
                            <xsl:value-of
                                select="round($char-per-cell div string-length($inp) * 100)" />
                        </xsl:when>
                        <xsl:otherwise>100</xsl:otherwise>
                    </xsl:choose>
                </xsl:otherwise>
    ...
    <xsl:template match="entry/screen">
        ...
        <xsl:choose>
            <xsl:when test="$shade.verbatim != 0">
                <fo:block font-size="{$font_size}%" id="{$id}" linefeed-treatment='preserve'
                    white-space-collapse='false'
                    white-space-treatment='preserve'
                    xsl:use-attribute-sets="monospace.verbatim.properties shade.verbatim.style">
                    <xsl:choose>
                        <xsl:when
                            test="$hyphenate.verbatim != 0 and function-available('exsl:node-set')">
