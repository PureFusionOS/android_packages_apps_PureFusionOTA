...
class pretty:
    def __init__(self, name):
        ...
        self.re_nsl = re.compile(
            r'^(\s+typedef\s+.*?;|\s*(private|public):\s*|\s*{\s*|\s*(\w|\d|,)+\s*)$')
        ...
        self.re_static_const = re.compile(
            r'(\s*)((BOOST_STATIC_CONSTANT\(\s*\w+,\s*|enum\s*\w*\s*{\s*)value\s*=)(.*?)([}|\)];)$')
    ...
    def process(self, line):
        ...
        if self.re_templ_decl.match(line) and self.re_typedef.match(self.prev_line) \
                or not self.re_empty_line.match(line) and self.re_closing_curly_brace.match(
                    self.prev_line) \
                or not self.re_empty_line.match(self.prev_line) \
                        and (self.re_namespace_scope_templ.match(line) \
                                     or self.re_namespace.match(
                                line) and not self.re_namespace.match(self.prev_line) \
                             ):
            line = '\n%s' % line
