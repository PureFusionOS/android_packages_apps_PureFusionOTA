      ...
      compounddef[
        ...
        not(contains(@id, 'src'))]">
            ...
            <xsl:if test="count(/doxygen/compounddef[@id = $srcid]) &gt; 0">
                ...
                <xsl:value-of select="$newline" />
                <xsl:apply-templates
                    select="/doxygen/compounddef[@id = $srcid]/detaileddescription" />
    ...
    <xsl:template match="ref[@kindref='member']">
        ...
        <xsl:choose>
            ...
            <xsl:when test="@external='reference.tags'">
                ...
                <xsl:choose>
                    <xsl:when
                        test="count(document('reference.tags')/tagfile/compound[@kind='class']/member[anchor=$anchor]) &gt; 0">
                        <xsl:for-each
                            select="document('reference.tags')/tagfile/compound[@kind='class']/member[anchor=$anchor]">
