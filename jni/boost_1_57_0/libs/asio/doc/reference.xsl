...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
    ...
    <xsl:template name="strip-ns">
        ...
        <xsl:choose>
            ...
            <xsl:when test="contains($name, '::') and contains($name, '&lt;')">
                <xsl:choose>
                    <xsl:when
                        test="string-length(substring-before($name, '::')) &lt; string-length(substring-before($name, '&lt;'))">
    ...
    <xsl:template name="cleanup-type">
        ...
        <xsl:choose>
            <xsl:when test="$type='void_or_deduced'">
                <xsl:text>``[link
                    boost_asio.reference.asynchronous_operations.return_type_of_an_initiating_function
                    ['void-or-deduced]]``
                </xsl:text>
            </xsl:when>
            <xsl:otherwise>
    ...
    <xsl:template match="bold" mode="markup">[*<xsl:apply-templates mode="markup" />]
    </xsl:template>


    <xsl:template match="emphasis" mode="markup">['<xsl:apply-templates mode="markup" />]
    </xsl:template>
    ...
    <xsl:template match="parameteritem" mode="markup">
        [[<xsl:value-of select="parameternamelist" />][<xsl:apply-templates
        mode="markup-nested" select="parameterdescription" />]]
    </xsl:template>
    ...
    <xsl:template match="simplesect" mode="markup">
        <xsl:choose>
            ...
            <xsl:when test="@kind='see'"></xsl:when>
    ...
    <xsl:template match="ref[@kindref='member']" mode="markup">
        <xsl:variable name="dox-ref-id" select="@refid" />
        <xsl:variable name="memberdefs"
            select="/doxygen//compounddef/sectiondef/memberdef[@id=$dox-ref-id]" />
        <xsl:choose>
            <xsl:when
                test="contains(@refid, 'namespaceboost_1_1asio') and count($memberdefs) &gt; 0">
                <xsl:variable name="dox-compound-name"
                    select="($memberdefs)[1]/../../compoundname" />
                <xsl:variable name="dox-name" select="($memberdefs)[1]/name" />
                <xsl:variable name="ref-name">
                    <xsl:call-template name="strip-asio-ns">
                        <xsl:with-param name="name"
                            select="concat($dox-compound-name,'::',$dox-name)" />
                    </xsl:call-template>
                </xsl:variable>
                <xsl:variable name="ref-id">
                    <xsl:call-template name="make-id">
    ...
    <xsl:template name="class">
        ...
        <xsl:apply-templates mode="markup" select="briefdescription" />
        <xsl:text>

        </xsl:text>

        <xsl:apply-templates mode="class-detail" select="templateparamlist" />
        <xsl:text></xsl:text>
        ...
        <xsl:apply-templates mode="markup" select="detaileddescription" />
    ...
    count(
      ...
      innerclass[@prot='public' and not(contains(., '_handler'))]) &gt; 0">
            ...
            [table
            [[Name][Description]]
            <xsl:for-each mode="class-table" select="
    ...
    innerclass[@prot='public' and not(contains(., '_handler'))]">
                ...
                <xsl:choose>
                    <xsl:when test="count(name) &gt; 0">
                        [[link boost_asio.reference.<xsl:value-of select="$class-id" />.
                        <xsl:value-of select="name" />
                    ...
                    <xsl:otherwise>
                        ...
                        [<xsl:value-of
                        select="(/doxygen//compounddef[@id=$type-ref-id])[1]/briefdescription" />]
                    </xsl:otherwise>
                </xsl:choose>
                ]
            </xsl:for-each>
        ...
        <xsl:if test="count(sectiondef[@kind='public-func' or @kind='public-static-func']) > 0">
            ...
            <xsl:for-each
                mode="class-table"
                select="sectiondef[@kind='public-func' or @kind='public-static-func']/memberdef">
                <xsl:sort select="name" />
                <xsl:variable name="name">
                    <xsl:value-of select="name" />
                </xsl:variable>
                ...
                <xsl:if
                    test="not($overload-position = 1) and not(briefdescription = preceding-sibling::*/briefdescription)">
                    <xsl:value-of select="$newline" />
                    <xsl:value-of select="$newline" />
                    <xsl:text></xsl:text>
                    <xsl:value-of select="briefdescription" />
        ...
        <xsl:if
            test="count(sectiondef[@kind='protected-func' or @kind='protected-static-func']) > 0">
            [heading Protected Member Functions]
            [table
            [[Name][Description]]
            <xsl:for-each
                mode="class-table"
                select="sectiondef[@kind='protected-func' or @kind='protected-static-func']/memberdef">
                <xsl:sort select="name" />
                <xsl:variable name="name">
                    <xsl:value-of select="name" />
                </xsl:variable>
        ...
        <xsl:if test="$class-name = 'io_service::service'">
            <xsl:if test="count(sectiondef[@kind='private-func']) > 0">
                ...
                [table
                [[Name][Description]]
                <xsl:for-each mode="class-table"
                    select="sectiondef[@kind='private-func']/memberdef">
                    <xsl:sort select="name" />
                    <xsl:variable name="name">
                        <xsl:value-of select="name" />
                    </xsl:variable>
        ...
        <xsl:if test="count(sectiondef[@kind='public-attrib' or @kind='public-static-attrib']) > 0">
            ...
            <xsl:for-each
                mode="class-table"
                select="sectiondef[@kind='public-attrib' or @kind='public-static-attrib']/memberdef">
                <xsl:sort select="name" />
                [
                [[link boost_asio.reference.<xsl:value-of select="$class-id" />.
                <xsl:value-of select="name" />
        ...
        <xsl:if
            test="count(sectiondef[@kind='protected-attrib' or @kind='protected-static-attrib']) > 0">
            [heading Protected Data Members]
            [table
            [[Name][Description]]
            <xsl:for-each
                mode="class-table"
                select="sectiondef[@kind='protected-attrib' or @kind='protected-static-attrib']/memberdef">
                <xsl:sort select="name" />
                [
                [[link boost_asio.reference.<xsl:value-of select="$class-id" />.
                <xsl:value-of select="name" />
        ...
        <xsl:if
            test="count(sectiondef[@kind='friend']/memberdef[not(type = 'friend class') and not(contains(name, '_helper'))]) &gt; 0">
            [heading Friends]
            [table
            [[Name][Description]]
            <xsl:for-each
                mode="class-table"
                select="sectiondef[@kind='friend']/memberdef[not(type = 'friend class') and not(contains(name, '_helper'))]">
                <xsl:sort select="name" />
                <xsl:variable name="name">
                    <xsl:value-of select="name" />
                </xsl:variable>
        ...
        <xsl:if test="count(sectiondef[@kind='related']/memberdef) &gt; 0">
            ...
            [table
            [[Name][Description]]
            <xsl:for-each mode="class-table" select="sectiondef[@kind='related']/memberdef">
                <xsl:sort select="name" />
                <xsl:variable name="name">
                    <xsl:value-of select="name" />
                </xsl:variable>
    ...
    <xsl:template name="class-members">
        ...
        <xsl:apply-templates
            mode="class-detail"
            select="sectiondef[@kind='public-type' or @kind='public-func' or @kind='public-static-func' or @kind='public-attrib' or @kind='public-static-attrib' or @kind='protected-func' or @kind='protected-static-func' or @kind='protected-attrib' or @kind='protected-static-attrib' or @kind='friend' or @kind='related']/memberdef[not(type = 'friend class') and not(contains(name, '_helper'))]">
            <xsl:sort select="name" />
            <xsl:with-param name="class-name" select="$class-name" />
            <xsl:with-param name="class-id" select="$class-id" />
            <xsl:with-param name="class-file" select="$class-file" />
        ...
        <xsl:if test="$class-name = 'io_service::service'">
            <xsl:apply-templates
                mode="class-detail"
                select="sectiondef[@kind='private-func']/memberdef[not(type = 'friend class') and not(contains(name, '_helper'))]">
                <xsl:sort select="name" />
                <xsl:with-param name="class-name" select="$class-name" />
                <xsl:with-param name="class-id" select="$class-id" />
                <xsl:with-param name="class-file" select="$class-file" />
    ...
    <xsl:template match="memberdef" mode="class-detail">
        ...
        <xsl:if test="$overload-count &gt; 1 and $overload-position = 1">
            ...
            <xsl:for-each select="../memberdef[name = $name]">
                <xsl:if
                    test="position() &gt; 1 and not(briefdescription = preceding-sibling::*/briefdescription)">
                    <xsl:value-of select="$newline" />
                    <xsl:apply-templates mode="markup" select="briefdescription" />
                    <xsl:value-of select="$newline" />
                </xsl:if>
                ...
                ``[link boost_asio.reference.<xsl:value-of select="$class-id" />.<xsl:value-of
                select="$id" />.overload
                <xsl:value-of select="position()" />
                <xsl:text></xsl:text>
                <xsl:value-of select="name" />]``(<xsl:apply-templates mode="class-detail"
                select="param" />)<xsl:if test="@const='yes'">const</xsl:if>;
                <xsl:text>`` [''''&amp;raquo;'''</xsl:text>
                <xsl:text>[link boost_asio.reference.</xsl:text>
                <xsl:value-of select="$class-id" />.<xsl:value-of select="$id" />.overload
                <xsl:value-of select="position()" />
                <xsl:text>more...]]``
                </xsl:text>
            </xsl:for-each>
        ...
        <xsl:if test="$overload-count &gt; 1">overload
        ...
        <xsl:if test="not(starts-with($doxygen-id, ../../@id))">
            <xsl:variable name="inherited-from"
                select="/doxygen/compounddef[starts-with($doxygen-id, @id)]/compoundname" />
    ...
    <xsl:template name="function">
        ...
        <xsl:choose>
            <xsl:when test="count(/doxygen//memberdef[@id=$doxygen-id]/templateparamlist) = 1">
                <xsl:apply-templates mode="class-detail"
                    select="/doxygen//memberdef[@id=$doxygen-id]/templateparamlist" />
            </xsl:when>
            <xsl:otherwise>
                <xsl:apply-templates mode="class-detail" select="templateparamlist" />
            </xsl:otherwise>
        ...
        <xsl:value-of select="name" />(<xsl:apply-templates mode="class-detail" select="param" />
        )<xsl:if test="@const='yes'">const</xsl:if>;
    </xsl:template>


    <xsl:template match="templateparamlist" mode="class-detail">
        <xsl:text></xsl:text>template&lt;<xsl:apply-templates mode="class-detail-template"
        select="param" />&gt;
    </xsl:template>
    ...
    <xsl:template match="param" mode="class-detail-template">
        ...
        <xsl:choose>
            ...
            <xsl:when test="declname = 'DatagramSocketService1'">
                <xsl:value-of
                    select="concat('``[link boost_asio.reference.DatagramSocketService ', declname, ']``')" />
            </xsl:when>
            ...
            <xsl:when test="declname = 'Protocol1'">
                <xsl:value-of
                    select="concat('``[link boost_asio.reference.Protocol ', declname, ']``')" />
            </xsl:when>
            <xsl:when test="declname = 'RawSocketService1'">
                <xsl:value-of
                    select="concat('``[link boost_asio.reference.RawSocketService ', declname, ']``')" />
            </xsl:when>
            <xsl:when test="declname = 'SeqPacketSocketService1'">
                <xsl:value-of
                    select="concat('``[link boost_asio.reference.SeqPacketSocketService ', declname, ']``')" />
            </xsl:when>
            <xsl:when test="declname = 'Signature'">
                <xsl:value-of select="declname" />
            </xsl:when>
            <xsl:when
                test="declname = 'SocketAcceptorService1' or declname = 'SocketAcceptorService2'">
                <xsl:value-of
                    select="concat('``[link boost_asio.reference.SocketAcceptorService ', declname, ']``')" />
            </xsl:when>
            <xsl:when test="declname = 'SocketService1' or declname = 'SocketService2'">
                <xsl:value-of
                    select="concat('``[link boost_asio.reference.SocketService ', declname, ']``')" />
            </xsl:when>
            ...
            <xsl:when test="declname = 'StreamSocketService1'">
                <xsl:value-of
                    select="concat('``[link boost_asio.reference.StreamSocketService ', declname, ']``')" />
            </xsl:when>
            ...
            <xsl:when test="count(declname) = 0"></xsl:when>
            <xsl:otherwise>
                <xsl:value-of
                    select="concat('``[link boost_asio.reference.', declname, ' ', declname, ']``')" />
            </xsl:otherwise>
        </xsl:choose>
    ...
    <xsl:template name="namespace-memberdef">
        ...
        <xsl:if test="$overload-count &gt; 1 and $overload-position = 1">
            ...
            <xsl:choose>
                <xsl:when
                    test="count(/doxygen/compounddef[@kind='group' and compoundname=$name]) &gt; 0">
                    <xsl:for-each
                        select="/doxygen/compounddef[@kind='group' and compoundname=$name]">
                        <xsl:apply-templates mode="markup" select="briefdescription" />
                        <xsl:text></xsl:text>
                    </xsl:for-each>
                </xsl:when>
            ...
            <xsl:for-each select="../memberdef[name = $unqualified-name]">
                ...
                ``[link boost_asio.reference.<xsl:value-of select="$id" />.overload
                <xsl:value-of select="position()" />
                <xsl:text></xsl:text>
                <xsl:value-of select="name" />]``(<xsl:apply-templates mode="class-detail"
                select="param" />);
                <xsl:text>`` [''''&amp;raquo;'''</xsl:text>
                <xsl:text>[link boost_asio.reference.</xsl:text>
                <xsl:value-of select="$id" />.overload
                <xsl:value-of select="position()" />
                <xsl:text>more...]]``
                </xsl:text>
            </xsl:for-each>
